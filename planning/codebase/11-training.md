...
## ▶ 2026-07-01 — ASSESSMENT ENGINE (server-authoritative; **MERGED to main + deploying** — engine inc1-3, Auditor-verified)
...
- **Launch dial:** lesson 4-of-8, topic-final 15-of-22 (tunable → 6/25 later).
- **MERGED 2026-07-01 (engine inc1-3):** foundation + DnD shuffle + suggested-time removal + SPA exam runner (`PooledAssessment`) + `TopicFinal` construct (self-nulls until a final pool exists). **Auditor engine-verify PASSED all 6** (server-authoritative draw+grade, keys-stripped-on-start, ban-at-loader, Q2 repo-file pools, 0082 sound+idempotent, no-client-trust + WP-A 404-not-leak; report `docs/audit/engine-verify.md`). Planning re-ran `assessment_engine.test.js` 10/10 + `schema:sync` validated 0082 applies cleanly (82 migrations, 0 skipped).
- **LOW fixed at merge (Auditor find):** pool loader now excludes ALL `_`-prefixed files (was `_readme`-only → `_fixture-demo.json` was a real pool reachable via direct API). Confirmed: 0 pools load now (fixture excluded).
- **Deferred → O46:** `schema.sql` not regenerated for 0082 — no `pg_dump` in Planning's env; migrations are the source of truth + 0082 auto-applies on deploy; CI dead (O43) so no schema-drift gate fires. Regenerate when PG client tools are installed.
- **inc4 (in flight, first C1/C2 split):** gated T01 retrofit — C1 authoring L01-L03 pools + research-log, C2 red-teams (author≠RT) + citation pre-check; **+ remove `fill-in-blank` mode**. Lands as its own merge; the SPA is built into `public/training/` at THAT merge (engine SPA is dormant/gated until content ships).