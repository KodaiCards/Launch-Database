...
- **Watcher — PERSISTENT content-aware `Monitor` (D021-ref4 + D025; SUPERSEDES the old exit-on-change). FIRST ACTION on every boot/reset:** arm via the `Monitor` tool (`persistent:true`) — it NEVER exits, so your task list shows **≥1 running task ("background task running")**, which keeps your cloud container alive (a reset drops tasks to 0 → stale). Exact command in `BOOT_PROMPTS.md`:
  `C1B=<c1-branch>; C2B=<c2-branch>; pm=""; p1=""; p2=""; while true; do git fetch origin main "$C1B" "$C2B" -q 2>/dev/null; cm=$(git rev-parse "origin/main:planning/threads/ceo.md" 2>/dev/null); a=$(git rev-parse "origin/$C1B:planning/threads/c1.md" 2>/dev/null); b=$(git rev-parse "origin/$C2B:planning/threads/c2.md" 2>/dev/null); if [ -n "$pm" ] && [ "$cm" != "$pm" ]; then echo "WAKE: Planning ruling (ceo.md)"; fi; if [ -n "$p1" ] && [ "$a" != "$p1" ]; then echo "WAKE: C1 posted"; fi; if [ -n "$p2" ] && [ "$b" != "$p2" ]; then echo "WAKE: C2 posted"; fi; pm="$cm"; p1="$a"; p2="$b"; sleep 600; done` — CONTENT-AWARE: wakes you ONLY when a thread that concerns you changes (Planning's ruling on ceo.md, or a C1/C2 post), NOT on every tip move (no echo from main-merges/integrations).
  On wake → `git pull origin main` + `git fetch` C1/C2, act (the Monitor never exits — no re-arm needed). **MANDATORY BASELINE (correctness never depends on the watcher): checkpoint-pull `git pull origin main` at boot, before + after every increment, before any long verify, and before reporting.**
...
- **Cascade:** relay the **same content-aware watcher (D025)** to C1/C2 on their threads (they watch YOUR branch for their own dispatch file, not the tip) — that's your job, not Planning's. (The Auditor is Planning's, not yours.)
