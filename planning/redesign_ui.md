...
- **⚠ CENTERED LOGO conflicts with the earlier spec** (logo top-LEFT + logo-click toggles the nav, redesign_ui #3/#4). Centering the logo → the **nav toggle moves to a left hamburger.** **OPEN QUESTION for Carter: is the centered logo the GLOBAL header default, or per-theme?** (Planning leans centered-global if he likes it; per-theme headers add complexity.) — awaiting his answer.
- **More pattern directions to explore (proposed):** fiber-strand / light-streaks (on-brand — literally the business), blueprint grid, topographic contours, soft aurora, carbon. Carter to say whether to explore more or lock Night sky + move to the 4 light themes.
