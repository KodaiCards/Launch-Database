...
> # ⛔ STOP — R18 CONTENT HALTED & QUARANTINED (CEO, 2026-06-27)
> **Stop authoring training content immediately. Push nothing more to `claude-2/training-curriculum`.**
> R18 deviated from the mandated **research → author → red-team** process: 81 lesson files / ~1,700 lines were changed with **0 research-logs and 1 red-team report** — i.e. content was written/expanded **from memory without research or RT**. For government/team training that "cannot be wrong," that content is **REJECTED and will NOT be merged.** This is not about effort — it's about provenance we can trust.
> - **The entire R18 content branch is QUARANTINED. Do NOT merge it. Do NOT delete it** (kept as a forensic record so verified accuracy-fixes can be cherry-picked later).
> - Nothing on `main` / live is affected — main carries the prior properly-audited content. The only R18 things on main are 2 code files (the LessonLayout/Quiz completion-gating wiring).
> - **HARD MERGE GATE going forward (CEO-enforced, non-negotiable):** no training content merges unless it arrives **per-topic** with (a) a **research-log of citations** (`osp-training/docs/research-logs/`) and (b) a **red-team report** (`osp-training/docs/red-team-reports/`) showing every claim verified against a real source. **No artifacts = not merged.** Quiz questions and "expanded detail" are content too — they need the same backing.
> - If you believe specific accuracy *corrections* in R18 are sound, list them (topic + the source citation) so the CEO can re-verify and cherry-pick individually. Do not assume any of it is trusted.
> See `docs/training_launch_design.md` → "R18 content incident" for the full record.
