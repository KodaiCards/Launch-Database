...
One table, per T-topic, tracking each quality dimension so nothing is missed: **research-log?** · **independent topic-level RT?** · **known live errors** (punch-list)? · **assessment** (≥4 Q, mixed types, cumulative/interleaved)? · **interactivity** (beyond MC; calculator/sim where it teaches)? · **SVG diagram(s)** for technical visuals? · **verbiage** (plain, jargon defined)? · **flashcards / `vocabulary_introduced` / `key_terms`** present? · **DAG prereq pointers** valid (no broken `vocabulary_assumed`)? Work **biggest-gap / highest-risk first**: (1) the live-error punch-list (Priority 0), (2) topics with no research foundation, (3) the rest. This guarantees nothing is missed and makes the whole-curriculum scope legible. Commit the matrix so the CEO can track coverage.
