(function(){'use strict';let currentUser=null;let currentFolderId=null;let selectedItems=new Set();document.addEventListener('DOMContentLoaded',async()=>{try{const meRes=await fetch('/api/auth/me',{credentials:'include'});if(meRes.status===401){window.location.href='/';return;}currentUser=await meRes.json();if(!currentUser.id)throw new Error('No user ID');await loadTree();await loadTrash();setupEventListeners();renderFileTable([]);}catch(err){console.error('Init failed:',err);showToast(`Failed to initialize: ${err.message}`,'error');}});async function loadTree(){try{const res=await fetch('/api/workspace/tree',{credentials:'include'});if(!res.ok)throw new Error(`HTTP ${res.status}`);const data=await res.json();const usersTree=document.getElementById('users-tree');usersTree.innerHTML='';if(data.users&&data.users.length>0){data.users.forEach(user=>{usersTree.appendChild(createTreeItem(user,'user'));})}else{usersTree.innerHTML='<div class="tree-item disabled"><small>No users</small></div>';}const sharedTree=document.getElementById('shared-tree');sharedTree.innerHTML='';if(data.shared&&data.shared.length>0){data.shared.forEach(folder=>{sharedTree.appendChild(createTreeItem(folder,'shared'));})}else{sharedTree.innerHTML='<div class="tree-item disabled"><small>No shared folders</small></div>';}}catch(err){console.error('Load tree failed:',err);showToast(`Failed to load tree: ${err.message}`,'error');}}function createTreeItem(folder,type){const div=document.createElement('div');div.className='tree-item';div.dataset.folderId=folder.id;div.dataset.folderName=folder.name;let icon='fa-folder';if(type==='user')icon='fa-user';else if(folder.kind==='shared_public')icon='fa-globe';else if(folder.kind==='shared_managers')icon='fa-lock';const hasChildren=folder.children&&folder.children.length>0;const toggle=hasChildren?`<span class="tree-toggle">▶</span>`:'<span class="tree-toggle"></span>';const renameIcon=type==='shared'?`<span class="tree-rename-btn" title="Rename folder"><i class="fas fa-pencil"></i></span>`:'';div.innerHTML=`${toggle}<i class="fas ${icon} tree-item-icon"></i><span class="tree-item-name">${escapeHtml(folder.name)}</span>${renameIcon}`;div.addEventListener('click',(e)=>{if(e.target.closest('.tree-toggle')){e.stopPropagation();toggleSubfolders(div,folder);}else if(e.target.closest('.tree-rename-btn')){e.stopPropagation();startFolderRename(div,folder.id,folder.name);}else{selectTreeItem(div,folder.id);}});return div;}function toggleSubfolders(treeItemEl,folder){const toggle=treeItemEl.querySelector('.tree-toggle');const isOpen=toggle.classList.contains('open');if(isOpen){toggle.classList.remove('open');toggle.textContent='▶';const childrenDiv=treeItemEl.nextElementSibling;if(childrenDiv&&childrenDiv.classList.contains('tree-children')){childrenDiv.remove();}}else{toggle.classList.add('open');toggle.textContent='▼';if(folder.children&&folder.children.length>0){const childrenDiv=document.createElement('div');childrenDiv.className='tree-children';folder.children.forEach(child=>{childrenDiv.appendChild(createTreeItem(child,'shared'));});treeItemEl.insertAdjacentElement('afterend',childrenDiv);}}}function startFolderRename(treeItemEl,folderId,currentName){const nameSpan=treeItemEl.querySelector('.tree-item-name');const renameBtn=treeItemEl.querySelector('.tree-rename-btn');if(!nameSpan)return;nameSpan.style.display='none';renameBtn.style.display='none';const input=document.createElement('input');input.type='text';input.className='tree-rename-input';input.value=currentName;input.maxLength=200;treeItemEl.insertBefore(input,nameSpan);input.focus();input.select();let isSaving=false;async function saveFolderName(){if(isSaving)return;const newName=input.value.trim();if(!newName){showToast('Folder name cannot be empty','error');cancelRename();return;}if(/[\/\\]/.test(newName)){showToast('Folder name cannot contain / or \\','error');return;}if(newName===currentName){cancelRename();return;}isSaving=true;try{const res=await fetch(`/api/workspace/folders/${folderId}`,{method:'PUT',credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:newName})});if(!res.ok)throw new Error(`HTTP ${res.status}`);showToast('Folder renamed','success');nameSpan.textContent=newName;currentName=newName;treeItemEl.dataset.folderName=newName;cancelRename();}catch(err){console.error('Rename failed:',err);showToast(`Rename failed: ${err.message}`,'error');}}function cancelRename(){input.remove();nameSpan.style.display='';renameBtn.style.display='';}input.addEventListener('keydown',(e)=>{if(e.key==='Enter'){saveFolderName();}else if(e.key==='Escape'){cancelRename();}});input.addEventListener('blur',saveFolderName);}function selectTreeItem(element,folderId){document.querySelectorAll('.tree-item.active').forEach(el=>{el.classList.remove('active');});element.classList.add('active');currentFolderId=folderId;loadFolderContents(folderId);updateDetailsPanel(folderId);}async function loadFolderContents(folderId){try{const res=await fetch(`/api/workspace/folders/${folderId}/files`,{credentials:'include'});if(!res.ok)throw new Error(`HTTP ${res.status}`);const files=await res.json();renderFileTable(files);updateBreadcrumb(folderId);updateButtonStates();}catch(err){console.error('Load folder failed:',err);showToast(`Failed to load folder: ${err.message}`,'error');}}// Track last selected row for shift-click range selection
let lastSelectedRow = null;

function renderFileTable(files){const tbody=document.getElementById('file-table-body');selectedItems.clear();if(!files||files.length===0){tbody.innerHTML='<tr class="loading-row"><td colspan="7" class="loading">Folder is empty</td></tr>';return;}tbody.innerHTML='';lastSelectedRow = null;files.forEach(file=>{const tr=document.createElement('tr');tr.className='file-row';tr.dataset.fileId=file.id;const isFolder=file.kind==='folder';const icon=isFolder?'fa-folder':getFileIcon(file.name);const typeLabel=isFolder?'Folder':'File';const size=isFolder?'—':formatBytes(file.size||0);const uploadedBy=file.uploaded_by||'—';const uploadedAt=formatDate(file.created_at);tr.innerHTML=`<td style="width:3rem;"><input type="checkbox" class="file-checkbox" data-file-id="${file.id}" /></td><td><i class="fas ${icon}"></i> <span class="file-name" onclick="window.openFolder&&window.openFolder('${file.id}')">${escapeHtml(file.name)}</span></td><td style="width:6rem;">${typeLabel}</td><td style="width:8rem;">${size}</td><td style="width:12rem;">${uploadedBy}</td><td style="width:10rem;">${uploadedAt}</td><td style="width:12rem;"><button class="btn btn-sm btn-secondary" onclick="window.downloadFile&&window.downloadFile('${file.id}')"><i class="fas fa-download"></i></button>${!isFolder?`<button class="btn btn-sm btn-secondary" onclick="window.showVersionHistory&&window.showVersionHistory('${file.id}')"><i class="fas fa-history"></i></button>`:''}</td>`;tr.addEventListener('click',(e)=>{if(e.target.closest('input[type="checkbox"]')){const checkbox=e.target;const isChecking=checkbox.checked;if(e.shiftKey&&lastSelectedRow){selectRangeOfRows(lastSelectedRow,tr,isChecking);}else if(e.ctrlKey||e.metaKey){if(isChecking){selectedItems.add(file.id);}else{selectedItems.delete(file.id);}}else{selectedItems.clear();if(isChecking){selectedItems.add(file.id);}document.querySelectorAll('.file-checkbox').forEach(cb=>cb.checked=cb===checkbox);}lastSelectedRow=tr;updateButtonStates();updateSelectionToolbar();}else if(!e.target.closest('button')){const checkbox=tr.querySelector('input[type="checkbox"]');const isChecking=!checkbox.checked;if(e.shiftKey&&lastSelectedRow){selectRangeOfRows(lastSelectedRow,tr,isChecking);}else if(e.ctrlKey||e.metaKey){checkbox.checked=isChecking;if(isChecking){selectedItems.add(file.id);}else{selectedItems.delete(file.id);}}else{selectedItems.clear();checkbox.checked=isChecking;if(isChecking){selectedItems.add(file.id);}document.querySelectorAll('.file-checkbox').forEach(cb=>cb.checked=cb===checkbox);}lastSelectedRow=tr;updateButtonStates();updateSelectionToolbar();}});tbody.appendChild(tr);});window.openFolder=loadFolderContents;window.downloadFile=downloadFile;window.showVersionHistory=showVersionHistory;window.downloadAllSelected=downloadAllSelected;window.deleteAllSelected=deleteAllSelected;window.clearSelection=clearSelection;}

function selectRangeOfRows(startRow,endRow,isChecking){const tbody=document.getElementById('file-table-body');const rows=Array.from(tbody.querySelectorAll('.file-row'));const startIdx=rows.indexOf(startRow);const endIdx=rows.indexOf(endRow);if(startIdx===-1||endIdx===-1)return;const min=Math.min(startIdx,endIdx);const max=Math.max(startIdx,endIdx);for(let i=min;i<=max;i++){const row=rows[i];const checkbox=row.querySelector('.file-checkbox');const fileId=row.dataset.fileId;checkbox.checked=isChecking;if(isChecking){selectedItems.add(fileId);}else{selectedItems.delete(fileId);}row.classList.toggle('selected',isChecking);}}function updateBreadcrumb(folderId){const breadcrumbItems=document.getElementById('breadcrumb-items');breadcrumbItems.innerHTML='';const folderName=findFolderNameInTree(folderId);if(folderName&&folderId!=='root'){const span=document.createElement('span');span.className='breadcrumb-separator';span.textContent=' / ';breadcrumbItems.appendChild(span);const link=document.createElement('a');link.className='breadcrumb-link';link.href='#';link.textContent=folderName;breadcrumbItems.appendChild(link);}}function findFolderNameInTree(folderId){const treeItems=document.querySelectorAll('.tree-item.active');if(treeItems.length>0){return treeItems[0].dataset.folderName;}return null;}function updateDetailsPanel(folderId){const detailsContent=document.getElementById('details-content');const folderName=findFolderNameInTree(folderId);detailsContent.innerHTML=`<div class="details-item"><label>Folder Name</label><p>${escapeHtml(folderName||'Unknown')}</p></div><div class="details-item"><label>Share Mode</label><select onchange=""><option value="private">Private</option><option value="public">Public</option><option value="specific">Specific Users</option></select></div>`;}function downloadFile(fileId){window.location.href=`/api/workspace/files/${fileId}/download`;}async function showVersionHistory(fileId) {
  try {
