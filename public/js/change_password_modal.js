...
(function () {
  ...
  function _inject() {
    ...
    wrap.innerHTML = `
      <div class="modal-overlay" id="cpm-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:2100;align-items:center;justify-content:center">
        <div style="background:var(--white,#fff);color:var(--text,#212529);border-radius:10px;padding:18px;max-width:420px;width:92%;box-shadow:0 10px 30px rgba(0,0,0,.3)">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
            ...
            <button onclick="closeChangePasswordModal()" style="background:transparent;border:0;color:var(--text-muted,#6c757d);font-size:18px;cursor:pointer">&times;</button>
          </div>
          <div style="display:flex;flex-direction:column;gap:10px">
            <div>
              <label style="font-size:12px;color:var(--text-muted,#6c757d);display:block;margin-bottom:4px">Current Password</label>
            ...
            <div>
              <label style="font-size:12px;color:var(--text-muted,#6c757d);display:block;margin-bottom:4px">New Password (min 6 chars)</label>
            ...
            <div>
              <label style="font-size:12px;color:var(--text-muted,#6c757d);display:block;margin-bottom:4px">Confirm New Password</label>
            ...
            <div id="cpm-error" style="color:#dc3545;font-size:12px;display:none"></div>
          </div>
          <div style="display:flex;justify-content:flex-end;gap:8px;margin-top:14px">
            ...
            <button onclick="submitChangePasswordModal()" style="padding:8px 14px;background:#1B5FA0;color:#fff;border:0;border-radius:6px;cursor:pointer;font-weight:600"><i class="fa-solid fa-key"></i> Update</button>
          </div>
        </div>
      </div>
    `;
  ...
  window.openChangePasswordModal = function () {
    ...
    document.getElementById('cpm-error').style.display = 'none';
