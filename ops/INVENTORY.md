...
| **Rudy Douglas (Director) initial grants** — RESOLVED (Carter 2026-07-14): = the `Douglas` account (`b346f865`, active). Registrar applies personal grants `cockpit.view`+`hours.view_all`+`projects.view_all` (NOT billing) at #73-core merge. A named "Director" role is out of System F scope (roles = hardcoded `VALID_ROLES` enum) — Carter raising self-serve role/perm *creation* with Partner. | #73 ruling + this ledger | seed applies at #73-core merge; role-creation → Partner |
