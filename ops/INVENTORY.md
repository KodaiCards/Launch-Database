...
| **#81 MERGE HELD pending #84** — #81's photo-endpoint unhang is `verified` but merging it makes #84's unscoped-photo-access leak reachable. Land #81 **with/after** #84's fix. DO NOT FORGET. | open #81 + #84 | #84's Partner-specced fix merges → then merge #81 |
| **#84 → PARTNER (System F reconciliation)** — Carter 2026-07-20 (L-016): photo-read access is admin-granted/editable/optioned, not baked → Partner specs a System F photos-read key + scoping options (parallel to #83); SQL `$2` fix becomes its enforcement. Marked `blocked`. Leak NOT prod-reachable (endpoints hang on prod; #81 undeployed). | open #84 (`blocked`) + `specs/roles-capabilities.md` | Partner rules photos-read key → decompose → build → then #81 unblocks |
