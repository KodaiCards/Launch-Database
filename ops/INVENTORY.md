...
| **training-fix (live-5) — WO-1 + WO-2·shared MERGED 2026-07-03** | 🟡 | Registrar merged #46 (WO-2 shared-infra: `ReferencesBlock` component + `course-catalog.js` natural-name rename) + #47 (WO-1 leak-strip on T01/T18/T02/T03/T04; VO-verified SHA `63de4640` only — WO-3 premerge commits excluded). Live-5 trainee-visible strings clean; residual `[confirm]`/`[verify]` hedges survive only in **non-rendered `meta.key_terms`** — WO-2 must resolve them when surfacing citations into References blocks. dist rebuilt at merge. | **WO-3 premerge #48 MERGED 2026-07-03** (`aff2eec2`): `npm run premerge` (build→lint→walk→tests) live. Lint on main = exit 1, 197 findings (leak 0, draw-count 0; gameability 24→#57, visible-id 173→#56/WO-2) = documented integration-gate redness, green-able once the content wave lands. In flight: WO-2 per-topic #50-54 (`fix-needed`), WO-4 #49 (`built`) — republish flip is Carter's after these clear. ⚠ premerge `npm test` stage can't run in Registrar env (DB-backed suite times out vs shared remote DB) — env gap flagged to Carter |
