...
| **System F permissions — `permission_grants` + `requirePermission` + catalog + custom-role bundles** | 🟡 | **MERGED 2026-07-14 (#73/#74/#75 closed; #76 backend):** resolver (base ∪ role ∪ personal ∪ bundle, admin=all) · grant/revoke + catalog API · Settings page grant surfaces · enforcement core (certificates.issue, money.manage_billing invoice gate, hours-override AUGMENT) · migrations 0084/0085 (L-016: zero baked grants). Partner default-grant seed RATIFIED + banked in `specs/roles-capabilities.md` (2026-07-15, cherry-pick 91dbfd14). **Owed:** #76 bundle-mgmt UI · #77 broad ~25-file delegation spread (builds the seed rows) · prod deploy/live-smoke (Railway down). | Partner rules role-default grants (L-016 editable rows) |
...
| **#81 + #84 land TOGETHER** — #81's photo-endpoint unhang (`verified`) can't ship without #84's scope fix or the leak goes live. Both on f1's #81 branch (same file). | open #81 + #84 | #84 built → `verified` + XFM PASS(f2) + premerge → merge #84+#81 together, at/after #77 seed |
| **#84 Partner ruling CORRECTED 2026-07-21 → f1 BUILT** — f1 found `job_assignments`(0032)/`ec_job_visibility`(0037) have NO per-user column (verified), so "scope to caller" was infeasible; Partner corrected: **REMOVE the two unscoped `EXISTS`**, gate broad read on **`projects.view_all`** via #73 resolver (contractor→own uploads+null-client = leak closed; staff hold view_all = no regression). Built on branch `claude/foreman-training-fix-u3lryh` (`04792f43` #81 + `d28aba16` #84, 13/13+8 tests). | open #84 (`claimed:f1`) + #81 | f1 flips `built` → vo1 `verified` + f2 XFM PASS + premerge → merge #84+#81 together, at/after #77 seed |
| **#85 admin Billing Ledger crash → RETIRE WITH CUTOVER** (Carter 2026-07-20) — #70 regression (deleted `billing.js` `/api/billing/report`; keystone handler shadows it → `admin.html:7749` TypeError). Not fixing; dies with the admin.html tree. | open #85 + `specs/cutover.md` / PLAN 2.3 | admin.html excision at cutover 2.3 (dies with the tree) |
| **#82 dashboard/overview $-leak → rides #77** — VALIDATED (hard rule 8 / PRODUCT_BAR §3): `service_areas.js:976` `requireStaff` leaks raw `$` SUMs to contractor/engineer. Durable fix = money-field strip on `money.view` (`omitUnless`), depends on #77's keys + re-gate. | open #82 + #77 | #77 verifies + merges → build #82 strip in the landing wave |
| **#86 one-time xfm backfill sweep** (Carter 2026-07-20) — consolidated cross-foreman user playthrough of packages merged since 07-13; preview vs `main` (NOT prod); one `bug` per finding. Umbrella tracker; split into two Carter-assigned lanes: **#87 `claimed:f1`** (f2's work: System F flows #73/#75/#76 + themes #69/#72) · **#88 `claimed:f2`** (f1's work: Settings #74 + post-#70 admin #70 + /verify+cert #67). | open #86/#87/#88 | Carter spins up f1+f2 → they confirm claims + branch → drive → file bugs → summary on #86 |
| ~~**`needs-xfm` GATES codification → PARTNER**~~ **LANDED 2026-07-20** (`499b2f1a`, Carter-signed `95a0b578`): GATES stack item 2 now MANDATORY w/ ≥2 foremen — builder labels `needs-xfm` at built-flip, peer posts `XFM PASS\|FAIL` + removes, Registrar refuses stamp without it. #86 = one-time backfill. Applied to **#77** (f1 is peer). | `law/GATES.md` §stack-2 | — (rule live; relayed to f1/f2/vo1 on #77) |
...
| **Training-diagram campaign (PLAN 1.6)** — spec landed `912e2df0` + **campaign-slot amendment `22528a6e`** (CONSTITUTION §5: +≤1 dedicated campaign role, ≤5 concurrent, does NOT displace f1/f2/vo1). Labels `graphic-idea`/`diagram-needed`/`claimed:il1`. Decomposed: **#89** infra primitive (unblocks all) → **#90** first diagram (dome closure → T01/L04) → **#91** retrofit sweep (live-5+T09). Owner = Diagram Illustrator @ **`claimed:il1`** (dedicated campaign slot). Runs alongside wave-2; retrofit live-5+T09 first, then born-illustrated. | `specs/diagrams.md` + #89/#90/#91 | Carter spins up the Illustrator (il1 campaign slot) → #89 → #90/#91 → campaign rolls; retires when curriculum illustrated |
...
| ~~**Role-default grants + invoice-gate policy**~~ **RESOLVED 2026-07-15:** Partner ratified the default-grant seed (`specs/roles-capabilities.md` §Ratified default-grant seed, 91dbfd14) — L-016-editable provenance, not baked. #77 builds these rows. | #75/#77 corrections | Partner ratified role-default rows → #77 applies as editable data |
...
| **#83 — reconcile `requireProjectCreate`/`user_portal_access` into System F** (`projects.create` key; preserve current creators as L-016 editable seed rows) — Partner ruling #77 item 5; `projects.js:246/517` left untouched by #77. Filed **BLOCKED**. | Partner #77 ruling + this ledger | Registrar flips #83 `blocked`→`open` when #77 lands |
| **`pricing.js` finer read-key** — `GET /api/pricing`,`/lookup` stay `requireStaff` now (rate auto-fill = daily engineer work; Partner #77 item 1). Introduce a finer read key when the rate catalog rebuilds. | Partner #77 ruling | 2.7 billing spec (rate-catalog rebuild) |
