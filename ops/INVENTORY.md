...
| **#84 RESOLVED by Partner (2026-07-20), Carter-locked → build (`claimed:f1`)** — photos follow existing **`projects.view_all`** (no new key, L-016-clean); scope both unscoped `EXISTS` to caller (copy `service_areas.js:833`) + `OR view_all` via #73 resolver; contractor over-reach closed. xfm peer = f2. Catalog housekeeping → #83 wave. | open #84 (`claimed:f1`) + #81 | same trigger as #81 row above |
