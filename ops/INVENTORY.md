...
| **training-fix (live-5) — WO-1 + WO-2·shared + WO-2 T02/T04 MERGED (2026-07-03→04)** | 🟡 | Registrar merged #46 (WO-2 shared-infra: `ReferencesBlock` + `course-catalog.js` rename) + #47 (WO-1 leak-strip; VO-verified SHA `63de4640` only). **2026-07-04: merged WO-2 #52 (T02 Fiber Physics, `4cbc5b7a`) + #54 (T04 Route Survey, `863e28f6`)** — clean 3-way merges on main (VO vo1 re-verify PASS + cross-foreman f1 PASS); #47 hedge-strip preserved (0 hedges post-merge); dist rebuilt (`5d779730`). **#50 (T01)/#51 (T18)/#53 (T03) NOT merged — 14-file WO-1↔WO-2 hedge/citation conflict; ESCALATED to Partner** (rebase-on-main + re-verify by f2/VO, not a Registrar hand-merge — hand-resolution = unverified trainee content, breaks independent-verification). | **WO-3 premerge #48 MERGED** (`aff2eec2`). Lint on main now exit 1, **134 findings (was 197): leak 0, draw-count 0; gameability 24→#57; visible-id 173→110** as T02/T04 raw-IDs clear. In flight: WO-2 #50/#51/#53 (conflict, escalated), WO-4 #49 (`fix-needed`/re-built for #58) — republish flip is Carter's after these clear. ⚠ premerge `npm test` stage can't run in Registrar env (DB-backed suite times out vs shared remote DB) — env gap flagged to Carter |
