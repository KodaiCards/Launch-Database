...
| **`needs-xfm` GATES codification → PARTNER** — the *standing* rule (any package skipping cross-foreman gets flagged + owes the lens) is law-level; lands in `law/GATES.md` via Carter × Partner, not Registrar. #86 is the one-time backfill only. | `law/GATES.md` + this ledger | Partner GATES amendment (batch with the #84 Partner session) |
