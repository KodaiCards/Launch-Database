...
| **Training-diagram campaign (PLAN 1.6)** — spec RATIFIED + landed `912e2df0` (`specs/diagrams.md` + style-guide + batch-1 reference SVG); labels `graphic-idea`/`diagram-needed` created. Decomposed: **#89** infra primitive (`AnnotatedDiagram`/`Figure` + sketch-filter, unblocks all) → **#90** first diagram (dome closure → T01/L04). Owner = Diagram Illustrator @ **`claimed:f3`** slot. Runs alongside wave-2; retrofit live-5+T09 first, then born-illustrated. | `specs/diagrams.md` + #89/#90 | Carter spins up the Illustrator (f3; cap f1+f2+f3+vo1=4) → #89 → #90 → campaign rolls; retires when curriculum illustrated |
