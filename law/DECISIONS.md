...
**L-015 (2026-07-05, backfilled 2026-07-14 — ruled and cited everywhere since, entry lost to a tooling failure) — Tests run ONLY on the disposable regression harness.** A local Registrar-only Postgres, recreated fresh per run (port 5433), never the shared DATABASE_URL (which the suite polluted for months — #62). Harness verifies CODE LOGIC; the live smoke on launchfiber.app verifies PROD — different tools, different questions, neither waives the other. *Why:* Carter chose option (a) after the prod-pollution discovery; closes the no-regression-net gap before money work.

**L-016 (2026-07-14) — NO BAKING: person-specific grants/config never ship in code, seeds, or migrations.** Feature and role DEFAULTS ship only as editable data rows — visible, changeable, and revocable on their admin surface exactly like user-created rows. The only coded permission behavior is the admin bootstrap (admin implicitly holds everything, so the first grant can be made). Applies to EVERYTHING it touches — permissions (#73–#77 incl. the #77 default-grant seed), mini-job templates, thresholds, rates, scoping defaults — current and future. VO lens on every package: a person-specific row in a migration/seed = FAIL; a "default" that cannot be changed on a page = FAIL. *Why:* Carter, verbatim: "seems stupid to bake a perm into code when its all fluid... let me just create a role, add perms to it and change perms individually for people. that simple." The system ships behavior; Carter ships policy.
