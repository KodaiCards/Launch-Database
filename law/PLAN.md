...
| 2.13 | **Desktop app** (resurrect `desktop/` Electron scaffold) — identity: **watched-folder local-file ingest → DB** (Workforce CSV, documents/photos first; KMZ + daily sheets when map/paperwork land). Sync model (Carter 2026-07-13): **real-time while connected; offline = capture queue + read-only cache, syncs on reconnect; money edits online-only** (offline replica of money data rejected — conflict risk vs hard rule 8) | specs/desktop-app.md | **pulled forward (Carter 2026-07-13)** — spec session may run ahead of Track-2 opening |
| 2.14 | **Mobile version (PWA-first, not native)** — the L-009 timeclock/daily-card flow (= 2.6's mobile flow) + **admin-lite** (approve hours, billing glance, nudges); offline punch queue. **Tab-list + scope spec session with Carter BEFORE any build (Carter 2026-07-13)** `*` | specs/mobile-app.md | timeclock flow lands with 2.6; admin-lite follows |
