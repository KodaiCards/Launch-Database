...
3. **OM1–OM5 multimode coverage — FIX a real gap.** Currently T02.L08 + T03.L05 cover SMF (G.652D, G.657A1/A2) but not multimode (OM1/OM2/OM3/OM4/OM5). Multimode shows up at building entrance, inside data centers, on short campus runs, and at every OSP↔ISP handoff. **This is not deferrable** — OSP engineers regularly terminate multimode at building entrance terminals. Either (a) add OM-series lesson to T03, or (b) patch T03.L05 to cover both SMF + MMF in depth. Architect decides which. **Self-correction note (Carter called this out 2026-05-16):** the coverage audit report flagged "No G.655 or OM-series multimode details" but I folded that into a positive framing instead of surfacing as a gap. Lesson logged in §3: when a coverage report says "covered, but missing X" — X gets a dedicated gap line, never buried in the positive summary.
...
22. **Citation-grounded research method (locked 2026-05-16 02:14 ET, Carter approved; 2-research default locked 2026-05-16 afternoon, Carter):** All OSP topic research goes through `audit-output/research-sources-allowlist.md`. Every research agent MUST cite from the allowlist with section/clause numbers. RT pair verifies citations EXIST + match claims. Paywalled sources (NESC, BICSI manuals, FOA full Reference Guide) require ≥2 research agents from DIFFERENT trusted-secondary sources for convergence + RT process-check on each researcher's logic chain. **Per-topic pipeline: ≥2 research covering the FULL topic scope (NOT split), with different FRAMINGS — e.g., R-A primary-source-first / high-precision / skeptical; R-B secondary-source-corroboration-first / high-recall / adversarial — → convergence check (overlap = verified; divergence = flag for orchestrator) → author (locked T02 template) → 2 RT same-scope/different-framings (topic verify pedagogy + technical).** Splitting scopes between research agents is a TRAP — it's faster coverage but loses cross-verification (Carter caught this 2026-05-16, repeat violation of the 2026-05-09 same-scope rule). Both research agents must see the WHOLE topic scope; framings differ in how they read sources, not what they read. 2-research is the floor, not the ceiling — paywalled-source-heavy topics get 3. Carter's verbatim 2026-05-16: *"I'm honestly concerned with bad data."* Going to single-research-per-topic is forbidden going forward. Cost delta is ~+30K Haiku / +60K Sonnet per topic — cheap insurance against bad citations / wrong section numbers / outdated standards editions getting baked into briefs that then propagate into authored lessons. **Retroactive check:** T01-T08 + T18 were authored with the old 1-research default; their post-author RT pairs validated content, but brief-stage citation misses (if any) won't have been caught. NOT urgent re-research — flag for audit only when a downstream RT finds a citation issue tracing back to brief stage. T01 + T02 get retroactive 3-iter research + 2 RT to verify locked-template accuracy (already queued separately). **Research model selection:** Sonnet default for OSP-curriculum research (high citation stakes + paywalled sources where Haiku hallucination history has been observed). Haiku only for true enumeration/lookup tasks where domain judgement isn't needed.
