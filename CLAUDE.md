...
- **EMPIRICAL SATURATION RULE for audit + RT dispatching (locked 2026-05-16 evening, Carter — refined by Carter same turn).** Standing rule for BOTH audit-phase research agents AND post-fix RT agents:
  - **Baseline floor (NEVER undershoot):** 2 audit/research agents for standard topics; 3 for paywalled-heavy topics (NESC/NEC/RUS-bulletin-dense); 2 RT minimum for post-fix waves. The saturation rule does NOT justify dispatching fewer than baseline.
  - **After baseline is met:** continue dispatching same-scope different-framing agents until the next agent returns either (a) no new findings OR (b) only findings that overlap with prior agents' reports. That's the saturation signal.
  - **Severity gate:** saturate on HIGH + MED. If the next agent's only new finds are LOW, route LOWs to Polish Queue (§4) and declare saturated. Otherwise indefinite dispatching for wording polish.
  - **Mechanical evaluation after each agent return:** compare its finding set against the union of prior agents' findings. If it added ANY new HIGH or MED → dispatch next round. If only LOW or rediscovered existing → stop + proceed to fix wave + capture LOWs to Polish Queue.
  - **Cost model:** Sonnet audit/RT ~75-160K per round; saturation across 3-5 rounds ~500-800K per high-stakes topic. For million-dollar-grade curriculum, correct trade.
  - **Empirical validation that locked this rule:** T05 audit (R-1/R-2/R-3 each found largely non-overlapping HIGH+MED) + T05 post-fix 1-RT-vs-2-RT (2-RT caught 4 NEW bugs single missed) + T18 R-1→R-2 each caught DIFFERENT HIGH safety bugs (methane density + nitrogen density + H₂S IDLH).
  - **Carter verbatim:** *"keep digging until we don't find more"* + *"don't let this rule override the initial 3 researchers and 2 RT we need at start."*
