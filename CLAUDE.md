...
22. **Citation-grounded research method (locked 2026-05-16 02:14 ET, Carter approved):** All OSP topic research goes through `audit-output/research-sources-allowlist.md`. Every research agent MUST cite from the allowlist with section/clause numbers. RT pair verifies citations EXIST + match claims. Paywalled sources (NESC, BICSI manuals, FOA full Reference Guide) require ≥2 research agents from DIFFERENT trusted-secondary sources for convergence + RT process-check on each researcher's logic chain. Per-topic pipeline: 1-2 research + 2 RT (brief) → author (locked T02 template) → 2 RT (topic verify pedagogy + technical). T01 + T02 get retroactive 3-iter research + 2 RT to verify locked-template accuracy.

23. **Agent-concurrency throttle (re-locked 2026-05-16 14:08 ET, Carter — now PERMANENT default):** Default operating mode = **single agent at a time, indefinitely**. Carter's verbatim — "Keep the throttle permanent with your discretion to temporarily change it for sprints or better efficiency or anything you decide. You have an extra 20% usage to play with for your discretion." So: ≤1 in-flight agent is the steady state from this lock forward. **Sprint exception (orchestrator discretion):** I can lift the throttle for a defined short burst when (a) work is genuinely parallel (different files, no push contention), (b) sequential burn would exceed acceptable wall-clock for the deliverable, or (c) the parallelism is a documented experiment per directive 20. **20% extra usage budget** allocated to orchestrator discretion — uses include sprints, faster RT pairs, retries on agent failure, experimental dispatches, anything that improves outcome or cost-efficiency. Track sprint use in chat ("burst N agents for X reason") so Carter sees the spend. After every sprint: orchestrator returns to single-agent default; no "recovery stint" required (rule simplified). Read-only chat tool calls (Bash, Read, Edit on CLAUDE.md) don't count toward the agent cap.
