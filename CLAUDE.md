...
23. **Agent-concurrency throttle (locked 2026-05-16 11:17 ET, Carter, after I burned 40% in ~1hr):** Single-agent mode for 1.5 hours from the lock (active until **12:47 ET 2026-05-16**). After the 1.5hr stint: ≤2 continuous agents in perpetuity unless a quick sprint is genuinely needed. Quick-sprint exception (>2 simultaneous agents for a defined short burst) requires a 1-hour single-agent recovery stint immediately after. Read-only chat tool calls (Bash, Read, Edit on CLAUDE.md) don't count toward the agent cap — but they still cost Opus tokens, so trim them too. Maintain the internal clock accurately on every meaningful turn so future-Claude can pick up the timer.
