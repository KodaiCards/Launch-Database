...
| T05 | NESC & Pole Loading | General | ⏳ RETROACTIVE RE-AUDIT IN PROGRESS — polish-3 + final-verify-2 pair pending | Original old-pipeline: Authors `31a81d9..6105c3e` / `e5fa80f..ee27415`, RT-A/B `70c8542` / `38ba195` (CRITICAL √2 math caught), fixes `eaae6ab..30470c6`. NEW PIPELINE this session: 3-agent audit (R-1 `3e1eb71`, R-2 `a6e922c`, R-3 `c55bf46`) + build fix-agent + post-fix RT pair + surgical patch wave 1+2 + polish-1 (`bef7e8c`) + final-verify RT pair (RT-A `634e6f8`, RT-B `3722497`) + polish-2 (`ffb9631`+`2ae77e1`). PENDING: polish-3 (1-line T07/L02 `existing utilities` source_lesson_id fix from polish-2 neighborhood scan) + final-verify-2 RT pair. |
| T18 | Safety & OSHA | General | ⏳ FULL RETROACTIVE — polish-3 + final-verify-3 pair pending (Z359.4 citation regression caught by RT-G/H) | Original old-pipeline: lessons `b51644f..e642a55`, RT-A `ec4c881`, RT-B `44736c3`, fix `96bfd9f`. NEW PIPELINE this session: 7-round audit (R-1 `00b277b`, R-2 `87e8980`, R-3 `b47b47c`, R-4 `01f84cd`, R-5 `93b7692`, R-6 `8df3e3f`, R-7 `a6b730b...` ROGUE SCOPE — applied 30 canonical fixes itself across 7 commits, work correct per spot-check) + verify RT pair RT-C `cf95bcb` YELLOW + RT-D `8a7a74d` GREEN + Haiku Gap-2 false positive resolution + polish-1 (`c34cb77`+`f4d351b`+`902a247`) + final-verify RT-E `4418399` GREEN + RT-F `af89e72` GREEN + polish-2 (`fed393c`+`d8c062c`) + final-verify-2 RT-G `3dbdd18` YELLOW + RT-H `397f54b` YELLOW. PENDING: polish-3 (Z359.4 → Z359.2 citation fix at 3 L04 locations + NEW-G2 LOW CO IDLH wording) + final-verify-3 RT pair. 4 HIGH safety bugs caught: methane density, nitrogen density, H₂S IDLH 100→50 ppm, LOTO entry-gate. Total burn ~4.4M. |
...
| T09 | Permitting & Environmental | General | ✓ AUTHORED (12 lessons on disk; §4 was stale — discovered 2026-05-16 during deploy) — RT pair under new pipeline PENDING | `audit-output/osp-rewrite-curriculum/T09_BRIEF.md` brief `6950a94`. Lesson files L01-L12 present in `osp-training/src/lessons/T09/`. Authored but no RT pair under new pipeline yet — back-fill queue. Brief: `6950a94`. |
