...
- **DEFAULT = 1 AGENT SEQUENTIAL (locked 2026-05-16 evening, Carter — supersedes prior 10% discretion budget for parallelism).** At most times run 1 agent. R-1 lands → dispatch R-2 → R-2 lands → Build → etc. The 10% discretion budget for parallel-safe work I'd been working with is RESCINDED. Carter's verbatim: *"At most times I'd run 1 agent, that's a good way to constrain usage."* Exception: cap-reset sprint with explicit "go faster" framing from Carter, OR genuinely-independent topics where wall-clock cost of sequential is materially higher than parallel cost. Default leans HEAVILY sequential.
