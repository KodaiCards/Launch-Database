...
| ARC-2 | orchestrator | `automation.js` `buildPscRusProjection` per-project pace | Per-project `pace × horizon` extrapolates 13 weeks even for near-complete projects. Overstates projection. **REVISED:** verification confirmed `projects.eta_date` does NOT exist + user confirms project ETAs are not tracked. | Use budget-burn heuristic instead of ETA column. Per-project: if `budget_remaining_hours > 0 AND (billed_to_date / budget_allocated) >= 0.80`, drop the effective horizon to 3 weeks (close-out window). Otherwise use full horizon. For projects without any budget set, use full horizon. Surface `horizon_reason` (`full` / `close_out_80pct`) in output for transparency. Do NOT add a `eta_date` column. |
...
| L-4 | verification new | `automation.js` `buildBillNowPreview` footage branch | Footage projects hardcode `already_billed = 0` and `earned = expected_revenue` — no deduction for prior partial invoices on footage projects. Could overstate unbilled footage revenue if a footage project has been partially invoiced. | Subtract `COALESCE(SUM(invoice_items.amount), 0)` for the project from the footage `earned` value, mirroring the hourly branch. |
