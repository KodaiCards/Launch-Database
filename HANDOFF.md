...
- **C2** — branch `claude-2/contractor-timeclock`, **Sonnet 4.6 / medium**, run by Carter as a separate instance. Built: Phase 5 hours/timeclock, contractor role support, Phase 4 money view, and the keystone operations cluster (clients/invoices/people/search/etc.). Done through **Round 7**; **Round 8 queued** (audit viewer, needs-attention panel, settings page, polish). **Near the additive ceiling.**
- **C3** — branch `claude/inspiring-bell-7jiwdf`, **Sonnet 4.6 / medium**, separate instance. Built: Phase 6 customer portal (`public/customer.html`), shim lifted + live. Done through **Round 6**; **Round 7 queued** (help/onboarding, contact, a11y/perf, print). **Near the additive ceiling.**
...
- **Merges follow `briefs/INTEGRATION.md`** (green/red gate). GREEN (frontend/tests/docs + CI green, no backend/schema) → Carter self-merges, free. RED (touches `routes/`, `server.js`, `auth.js`, `migrations/`, `schema.sql`, money/auth/customer-$/keystone-core, new endpoints, CI red, or a `BLOCKED` note) → **you** review.
