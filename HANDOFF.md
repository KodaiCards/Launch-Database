...
**⏳ AUDIT IN PROGRESS (2026-06-27) — the immediate active work.** The CEO is running an **evidence-based content audit** of the live curriculum before C6 touches anything, because `training_build_plan.md` was written *blind*. Reporter agents read all lessons and write durable files to `docs/audit/raw/audit-{A..F}.md`; the **CEO synthesizes** into the real coverage matrix + the exact grounded C6 plan + a VERIFY punch-list + a broken-tool repair list. **Full plan, the anti-hallucination protocol, and CONTINUATION instructions: `docs/audit/AUDIT_PLAN.md`.** **If you are a new CEO mid-audit: read `docs/audit/AUDIT_PLAN.md` → "CONTINUATION", check which `raw/` reports exist, re-launch any missing agent, spot-check quotes, then synthesize.** Carter flagged some interactive tools are broken — the audit includes a component health check.

**The worker situation:** C2 quarantined, C5 burned out. **C6 is the fresh content/RT instance — on a TIGHT LEASH** (`briefs/claude-6.md`): does **ONE small assigned piece, then STOPS and waits for Carter's explicit confirmation.** **C6 is HELD until the audit produces the grounded plan** — do not start it on content yet. Once the plan exists, C6's pieces are: T13 live-citation fixes (gated) → teaching-order topics one small unit at a time, each gated. Branch `claude-6/training-curriculum` off clean main. The CEO owns the vision/design + builds net-new components + the exemplar; C6 only pours **verified** content in.
...
> **PROGRESS (2026-06-24, later):** Cutover decisions settled (`docs/cutover_inventory.md`). Backends shipped + dev-tested this session (UIs fanned to C2 R11–R13): client/EC management UI (C2 R10, **merged**); **hours-CSV importer** re-pointed to `service_area_jobs` (`routes/hours_import.js` + `_hours_match.js`; UI R11); **billing→keystone** (migration `0066` + `routes/billing_keystone.js` progressive ledger; UI R12); **projections** (migrations `0067`/`0068` + `routes/projections.js`: expected−billed + RUS budget burn + map data + completed→final lifecycle + `bill_trigger`; design `docs/projections_design.md`; UI R13). **IA: projections = Money tab, overall map = Service Areas tab, no new rail items.** **Migrations 0066/0067/0068 applied to DEV only; prod apply + `schema.sql` regen (needs pg_dump16) pending for cutover.** Remaining: budgets rework (#4), permits/inspection tracking (#5 non-projection part), then admin cutover. Billing design `docs/billing_keystone_design.md`.
>
> **MAP→$ LOOP COMPLETE + LOCK-IN STATUS (2026-06-24):** All 4 loop increments shipped on `main` + dev-tested: (1) SA↔map-plan link + `/api/service-areas/:id/map-rollup`; (2) dual designation on spans → `footage_by_designation`; (3) map construction feeds the SA projection (`construction`+`combined` blocks, shared `routes/_map_estimate.js`); (4) mileage allocation `/api/projections/service-area/:id/mileage` (per-mile = remaining$ ÷ remaining miles). **DB LOCK-IN — there is ONE Railway DB** (`metro.proxy.rlwy.net:44516/railway`; the public URL Carter gave == the `.env` host:port:db exactly). **0066–0071 are VERIFIED applied to it** (tables construction_contracts/cost_catalog/map_store/contract_allocations + cols bill_trigger/map_plan_id/miles/construction_contract_id/closed_at/total_miles all present). So the DB side of lock-in is DONE — nothing to migrate. **This single DB currently holds only DEMO data** (DEMO — PSC / DEMO — Live Loop; 0 staff; ~$7.3k test invoices) → real PSC/RUS production data is NOT loaded into the keystone yet (open Q: is real data import a later step, or is there a truly separate real-data prod DB Carter hasn't pointed at?). **`schema.sql` regen — DEFERRED (Carter's call 2026-06-24, non-urgent).** It's stale (missing the new tables), but it's a dev-hygiene snapshot only: the live DB already has every migration applied + verified, and CI is billing-locked so nothing checks drift. Regen is a 30-sec `npm run schema:sync` on a machine that has **pg_dump 18** (server is PG 18.4 now — the old "pg_dump16" note is stale; this box has no pg_dump at all). Do it WHEN CI billing returns or a fresh DB needs bootstrapping — don't bug Carter about it before then. Note: Railway `startCommand` skips auto-migrate; budgets rework (`service_area_jobs.budget_code_id`) was designed but NOT built (no migration). Carter expects review + likely changes after.
>
> **MAP — standing directive (Carter 2026-06-24):** the map is the source of truth (units, mileage, footage, permitting all derive from it). **CEO maintains `docs/map_requirements.md`** — the living spec of what the map must do + a delivery diagnostic. When Carter delivers the map, diff it against that doc and build/change the map to fit. **Budgets/projections design is locked** (`docs/budgets_design.md`, `docs/projections_design.md`): two tiers — *simple SAs* (no EC/CC) project from job expected (built); *contract-based SAs* use **per-mile hourly allocation** + **per-CC unit-cost catalog** + a clean new **`construction_contracts`** entity + `service_areas.miles` + per-discipline budget/miles "area" on EC+CC. **The contract-allocation engine is intentionally NOT built yet** — its inputs (units/miles/footage) come from the map; build it with/just before the map to avoid blind rework. Interim inputs are manual.
