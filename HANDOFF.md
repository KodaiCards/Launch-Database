...
>
> **MAP→$ LOOP COMPLETE + LOCK-IN STATUS (2026-06-24):** All 4 loop increments shipped on `main` + dev-tested: (1) SA↔map-plan link + `/api/service-areas/:id/map-rollup`; (2) dual designation on spans → `footage_by_designation`; (3) map construction feeds the SA projection (`construction`+`combined` blocks, shared `routes/_map_estimate.js`); (4) mileage allocation `/api/projections/service-area/:id/mileage` (per-mile = remaining$ ÷ remaining miles). **CONFIRMED: `.env` `metro.proxy.rlwy.net` is the DEV DB** (only DEMO clients) — migrations **0066–0071 are on DEV only**. **PROD is a separate Railway DB, NOT yet migrated; CEO has no prod creds.** `schema.sql` is STALE (missing the new tables) — regen needs `pg_dump` (not on the Windows box). **Lock-in = Carter's action:** `DATABASE_URL=<prod> npm run migrate` (idempotent) → `npm run schema:sync` + commit → verify. Railway `startCommand` skips auto-migrate, so deploy alone won't apply them. Carter expects review + likely changes after.
>
