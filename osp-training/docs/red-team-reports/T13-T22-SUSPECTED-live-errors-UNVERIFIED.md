<!--
⚠️ UNVERIFIED PUNCH-LIST — produced by the QUARANTINED R18 attempt (see docs/training_launch_design.md → "R18 content incident — QUARANTINED").
These are SUSPECTED accuracy issues in LIVE content. The FLAGGED ERRORS were confirmed PRESENT in live lessons on main
(e.g. "Format V", AIA §3.3.1, NESC §01C / Section 26, NESC Map 1) — so they are real things to check. BUT the proposed
CORRECTIONS below come from the deviating instance and are NOT trusted. Each must be INDEPENDENTLY re-verified against the
cited authoritative source before any live edit. This is a punch-list for Claude-5 to verify-and-fix THROUGH THE GATE
(research-log + independent red-team). DO NOT hand-apply these corrections. — CEO 2026-06-27
-->
