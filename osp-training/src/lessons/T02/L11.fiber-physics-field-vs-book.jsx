// Net-new content — field gotchas and real-world variation not in source modules
...
export const meta = {
  id: 'T02.L11',
  course_id: 'T02',
  title: 'Fiber Physics in the Field vs. the Book',
  order: 11,
  lesson_type: 'working',
...
export default function T02L11_FiberPhysicsFieldVsBook() {
  return (
    <LessonLayout meta={meta}>
      ...
      <section data-tier="foundations">
        ...
        <p className="mt-2">
          By the end of this lesson, you'll be able to look at a high-loss OTDR event and have
        ...
        <p className="mt-2">
          A reel of G.652.D fiber is labeled "0.21 dB/km @ 1550 nm typical." That number is
          from the factory, measured under controlled conditions. On the actual job:
        </p>
        <ul className="list-disc pl-5 space-y-2 mt-2">
          <li>
            <strong>Temperature effects:</strong> Fiber attenuation changes with temperature.
            In extreme cold (−40°C), G.652.D attenuation can increase by 0.01–0.05 dB/km.
            In direct sun exposure on aerial runs, temperature cycling causes the cable to
            expand and contract — which can shift splices in their trays and induce stress.
            The 3 dB safety margin in a link budget exists partly to absorb seasonal variation.
          </li>
          <li>
            <strong>Cable installation tension:</strong> Pulling tension that's too high
            can permanently elongate the fiber, increasing microbend loss. Most OSP cables
            spec a maximum pull tension (typically 600–2700 N depending on cable design) —
            exceed it and the "factory" attenuation number no longer applies.
          </li>
          <li>
            <strong>Fiber aging:</strong> Glass is stable, but connector surfaces degrade,
            splice cases can be stressed by water intrusion or frost heave, and lashing
            wire can corrode and tighten on the cable. Real-world links tend to get slightly
            lossier over 20–30 years, which is why the safety margin must survive the
            link's design life.
          </li>
          <li>
            <strong>Batch variation:</strong> Even within G.652.D spec, two different
            production runs of the "same" fiber can have different typical attenuation.
            This matters when extending an existing link with a new reel — the OTDR will
            show an apparent "event" at the transition between old and new fiber even if
            both are in-spec.
          </li>
        </ul>
      </section>
      ...
      <section data-tier="working">
        <h2>The High-Attenuation Troubleshooting Checklist</h2>
        <p>
          When an OTDR shows higher loss than expected on a new installation, work through
          these in order of probability:
        </p>
        ...
        <ol className="list-decimal pl-5 space-y-3 mt-3">
          ...
          <li>
            ...
            Re-read the splice on a different splicer, or pull the pigtail and re-splice.
            Check for contamination on the fiber end-faces before the cleave.
          </li>
          <li>
            <strong>Wrong fiber type in the run</strong> (rare but catastrophic when it happens).
            If someone accidentally patched OM4 into the path of an OS2 run, the OTDR will
            show a huge step loss at the transition. Pull the patch panel cords and verify
            fiber type by color and label.
          </li>
          <li>
            <strong>Damaged section from pull tension</strong> (rare on new installs, more
            common on re-used conduit with sharp bends). The OTDR will show distributed
            high loss across the pulled section, not a discrete event.
          </li>
          <li>
            <strong>Water in a splice case</strong> (on in-service links). Water intrusion
            raises loss on specific fibers and can cause intermittent faults with temperature
            changes (water freezing/expanding can crack fiber). Ice-related break events
            are distinct on an OTDR — high back-reflection and complete signal cutoff.
          </li>
        </ol>
        ...
        <ul className="list-disc pl-5 space-y-2 mt-2 text-sm">
          <li>
            <strong>Splicing G.657 to G.652.D:</strong> G.657.A1 is designed to be backward
            compatible with G.652.D for splicing. MFD differences are small but non-zero.
            Expect ≤ 0.05 dB of additional splice loss at the transition versus G.652.D/G.652.D.
            G.657.A2 has slightly more MFD deviation — check the fiber spec sheet.
          </li>
          <li>
            <strong>G.657.B2/B3 is NOT fully compatible with G.652.D:</strong> Higher MFD
            mismatch. If you're extending a G.652.D backbone with a G.657.B3 drop, you may
            see unexpected splice loss. Don't mix without checking.
          </li>
          <li>
            <strong>"Bend-insensitive" doesn't mean "abuse-proof":</strong> G.657 tolerates
            tighter bends without optical loss, but it still has mechanical bend limits for
            the cable. A bend-insensitive fiber installed at 5 mm radius inside a stapled
            drop cable path will eventually fail mechanically even if it doesn't fail
            optically right away.
          </li>
        </ul>

        <div className="mt-5 p-4 border border-amber-400/30 bg-amber-400/5 rounded-lg text-sm">
          <p className="font-semibold text-amber-300 mb-1">One Rule for the Field</p>
          <p className="text-slate-300/90">
            When an OTDR shows unexpected loss, always <strong>clean and re-inspect the
            connector</strong> first. 70%+ of "fiber problems" in the field are actually
            contaminated connectors or connector mismatches. A proper end-face inspection
            scope and a dry + wet cleaning procedure resolves most issues before you
            start pulling cable or re-doing splices.
          </p>
        </div>
      </section>
      ...
      <section data-tier="advanced">
        <h2>Going Deeper — Why Aerial Fiber Ages Differently Than Underground</h2>
        <p>
          Aerial OSP fiber ages differently from direct-buried or conduit fiber:
        </p>
        <ul className="list-disc pl-5 space-y-2 mt-2 text-sm">
          <li>
            <strong>Thermal cycling:</strong> Aerial cable sees seasonal temperature swings
            of 100°F+ in many regions. Each cycle expands and contracts the cable and the
            fiber inside. Loose-tube cable handles this by letting the fiber "float" inside
            the gel-filled tube — the tube accommodates length change without stressing the
            fiber. Over time, aggressive thermal cycling can cause fiber to migrate within
            the buffer tube and create low-level microbend sections.
          </li>
          <li>
            <strong>UV degradation:</strong> The outer jacket absorbs UV over years. Jackets
            are formulated with UV stabilizers (HDPE, LLDPE), but eventually the jacket
            becomes brittle, especially in high-UV environments (south-facing aerial runs,
            desert climates). A brittle jacket that cracks under thermal stress can allow
            moisture ingress. Carbon-black compound in the jacket (standard for black OSP
            cable) provides significant UV protection but is not indefinite.
          </li>
          <li>
            <strong>Sag increase:</strong> Messenger wire creep (especially galvanized steel
            stranded messenger) causes sag to increase slowly over years. Higher sag doesn't
            directly affect fiber attenuation, but it changes pole loading and clearance.
            Check sag at the design recheck intervals per NESC.
          </li>
          <li>
            <strong>Direct-buried / conduit:</strong> More protected from thermal cycling
            extremes, but exposed to moisture and earth movement. Frost heave in northern
            climates can induce localized stress on direct-buried cable. Conduit corrugated
            HDPE provides good protection but rodent pressure on the jacket is a persistent
            threat in non-armored configurations.
          </li>
        </ul>
      </section>
      ...
      {/* ── BRANCHING SCENARIO ──────────────────────────────────────────── */}
      <BranchingScenario
        scenarioId="T02-L11-scenario-1"
        title="Field Scenario: Unexplained High Attenuation on a New Underground Run"
        description="A new 3-km underground SMF run shows 2.3 dB total loss on the OTDR acceptance test. The link budget allocates 0.9 dB for fiber (3 km × 0.30 dB/km conservative), 0.3 dB for 2 splices, and 0.6 dB for 4 connector pairs = 1.8 dB expected. The measured 2.3 dB is 0.5 dB over budget. What do you investigate?"
        startNodeId="start"
        nodes={{
          start: {
            id: 'start',
            prompt: 'The OTDR shows 2.3 dB total loss vs 1.8 dB expected. Where do you start?',
            choices: [
              {
                label: 'Assume the fiber is defective and request a cable replacement',
                consequence: 'This is premature. The excess is only 0.5 dB — well within the range of a dirty connector or a slightly tight bend. Cable replacement is a major and expensive step for what might be a 5-minute fix.',
                nextId: 'checkConn',
                isOptimal: false,
              },
              {
                label: 'Clean and re-inspect all four connector pairs, then re-run the OTDR',
                consequence: 'After cleaning: two of the four connector pairs were dirty — they were testing at 0.45 dB each instead of the expected 0.30 dB. Re-cleaning dropped them to 0.28 dB each. New total: approximately 0.9 + 0.3 + (0.28×4) = 2.32 dB. Wait — that\'s not enough. Still slightly over. But now you know connectors are clean.',
                nextId: 'checkBends',
                isOptimal: true,
              },
            ],
          ...
          checkConn: {
            id: 'checkConn',
            prompt: 'You\'re directed to check connectors first before escalating. You clean all four pairs and re-run the OTDR.',
            choices: [
              {
                label: 'Two connectors were dirty. After cleaning, loss drops from 2.3 to 2.0 dB. Still 0.2 dB over. Continue.',
                consequence: 'Good. Connectors were part of the problem. 0.2 dB remains unexplained. Now check the route.',
                nextId: 'checkBends',
                isOptimal: true,
              },
            ],
          ...
          checkBends: {
            id: 'checkBends',
            prompt: 'After cleaning connectors, there\'s still 0.2 dB of unexplained extra loss distributed across the run (not a discrete event). What next?',
            choices: [
              {
                label: 'Accept the result — 0.2 dB over budget is within measurement uncertainty',
                consequence: 'Possibly acceptable, but check the actual measurement uncertainty first. At 0.25 dB/km for 3 km, the fiber budget was estimated conservatively at 0.30 dB/km — the actual fiber is likely running 0.22–0.25 dB/km. The discrepancy may be planning conservatism, not a real defect. Document and close.',
                nextId: 'done',
                isOptimal: true,
              },
              {
                label: 'Run the OTDR at 1625 nm to check for microbend loss in the conduit section',
                consequence: 'Good technique. At 1625 nm, one 200 m section shows elevated loss — likely conduit overfill or over-tensioned pull in that segment. The contractor confirms the innerduct fill was slightly over spec. The excess 0.2 dB at 1550 nm is consistent with mild microbend. Documented in the as-built. Acceptable for this link budget.',
                nextId: 'done',
                isOptimal: true,
              },
            ],
