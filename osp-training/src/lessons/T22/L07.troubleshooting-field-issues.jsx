  ...
  return (
    <LessonLayout meta={meta}>
      ...
      <section data-tier="advanced">
        ...
        <h3 className="mt-5 font-semibold">Troubleshooting Decision Tree</h3>
        <div className="mt-3 text-sm space-y-2 text-slate-300/90">
          <div className="rounded bg-white/5 p-3">
            <p className="font-semibold text-blue-300">Step 1: Does any light reach the far end?</p>
            <p className="mt-1 ml-3">→ <strong>No light at all:</strong> Hard break or disconnected connector. Proceed to OTDR.</p>
            <p className="mt-1 ml-3">→ <strong>Some light but below threshold:</strong> High-loss event. OTDR to locate.</p>
            <p className="mt-1 ml-3">→ <strong>Light present but intermittent:</strong> Loose connector or temperature-sensitive micro-fracture.</p>
          </div>
          <div className="rounded bg-white/5 p-3">
            <p className="font-semibold text-blue-300">Step 2: OTDR from both ends. Locate the event.</p>
            <p className="mt-1 ml-3">→ <strong>Flat line (no return):</strong> Hard break — travel to that location, inspect physically.</p>
            <p className="mt-1 ml-3">→ <strong>Sharp downward step at known splice:</strong> High-loss splice — verify with bidirectional average, then re-splice if confirmed.</p>
            <p className="mt-1 ml-3">→ <strong>Elevated loss over a span (no single event):</strong> Macrobend — trace cable for tight coils.</p>
            <p className="mt-1 ml-3">→ <strong>Event visible from one direction only:</strong> Dead zone artifact or directional asymmetry — re-measure from the opposite end more carefully.</p>
          </div>
          <div className="rounded bg-white/5 p-3">
            <p className="font-semibold text-blue-300">Step 3: Decide on fix.</p>
            <p className="mt-1 ml-3">→ <strong>Hard break:</strong> Understand root cause (rock, vehicle, rodent). Replace section + address root cause.</p>
            <p className="mt-1 ml-3">→ <strong>High-loss splice:</strong> Re-splice. Confirm improved result with OTDR from both ends.</p>
            <p className="mt-1 ml-3">→ <strong>Macrobend:</strong> Find the tight coil, straighten and re-support. Re-test.</p>
            <p className="mt-1 ml-3">→ <strong>Loose connector:</strong> Clean with IPA + lint-free wipe. Re-seat. If still bad, replace.</p>
          </div>
        </div>

        <h3 className="mt-5 font-semibold">Replace vs. Re-Splice Decision</h3>
        <table className="w-full text-sm border border-white/10 rounded-lg mt-3">
          <thead className="bg-white/5 text-slate-200">
            <tr>
              <th className="px-3 py-2 text-left">Scenario</th>
              <th className="px-3 py-2 text-left">Replace section?</th>
              <th className="px-3 py-2 text-left">Re-splice?</th>
              <th className="px-3 py-2 text-left">Rationale</th>
            </tr>
          </thead>
          <tbody className="text-slate-300/90 text-xs">
            <tr className="border-t border-white/10">
              <td className="px-3 py-2">Hard break, &lt;50 m from an existing splice</td>
              <td className="px-3 py-2">No</td>
              <td className="px-3 py-2">Yes — add short patch, re-splice</td>
              <td className="px-3 py-2">Cheaper than replacing a full cable reel</td>
            </tr>
            <tr className="border-t border-white/10">
              <td className="px-3 py-2">Hard break, &gt;500 m from any splice</td>
              <td className="px-3 py-2">Yes — cut out damaged section, splice in new cable</td>
              <td className="px-3 py-2">N/A</td>
              <td className="px-3 py-2">Cannot re-splice a broken fiber without new cable</td>
            </tr>
            <tr className="border-t border-white/10">
              <td className="px-3 py-2">High-loss splice (&gt;0.3 dB)</td>
              <td className="px-3 py-2">No</td>
              <td className="px-3 py-2">Yes — clean, re-cleave, re-fuse</td>
              <td className="px-3 py-2">Re-splicing is far cheaper; second splice almost always better</td>
            </tr>
            <tr className="border-t border-white/10">
              <td className="px-3 py-2">Recurring breaks in same cable section</td>
              <td className="px-3 py-2">Yes — replace the entire section</td>
              <td className="px-3 py-2">No</td>
              <td className="px-3 py-2">Repeated breaks = cable degradation; patching delays the inevitable</td>
            </tr>
          </tbody>
        </table>

        <h3 className="mt-5 font-semibold">Worked Scenario: OTDR Investigation</h3>
        <div className="rounded bg-white/5 p-3 mt-3 text-sm space-y-2 text-slate-300/90">
          <p><strong>Situation:</strong> A 12-mile buried ADSS trunk goes down. No light at far end. OTDR from end A shows a flat line starting at 6.2 miles. OTDR from end B shows a large downward step at 5.8 miles from end B, then flat.</p>
          <p><strong>Analysis:</strong> 6.2 + 5.8 = 12.0 miles (total span). Both readings point to a break at approximately 6.0 miles from end A — the difference is dead zone and trace uncertainty. This is a hard break.</p>
          <p><strong>Field investigation:</strong> Crew travels to the 6-mile mark. Finds a recently disturbed area — a contractor dug a utility trench crossing the cable route without calling 811. Cable is severed cleanly.</p>
          <p><strong>Root cause:</strong> Unauthorized third-party excavation (dig-in).</p>
          <p><strong>Fix:</strong> (1) Document location and photograph the scene (legal record for damages). (2) Splice in a 50-ft replacement section with two new fusion splices. (3) OTDR from both ends: both new splices read ≤0.1 dB. (4) Restore service. (5) Submit damage claim to the contractor's insurance.</p>
          <p><strong>Lesson:</strong> The bidirectional OTDR gave exact location before the crew drove out — saving time. Root cause documentation was essential for the damage claim. Had they just patched without documentation, the third party escapes liability.</p>
        </div>
      </section>
      ...
      <section className="mt-8 rounded-lg bg-slate-800/40 border border-slate-700 p-4">
        ...
        <p className="text-sm text-slate-300/90 mt-2">
          In <strong>T22.L04</strong> you learned how to read an OTDR trace during acceptance testing — recognizing events, measuring loss, comparing to budget.
          Troubleshooting is that same skill applied under pressure: the link is down, a customer is offline, and you need to locate the fault fast.
          The three-step process (light present? → OTDR from both ends → decide fix) converts a panic situation into a systematic workflow.
          Understanding root cause — not just patching the symptom — prevents the same fault from recurring.
          Every troubleshooting event also generates documentation: OTDR traces, photos, corrective action log. That documentation protects the contractor from liability and helps the borrower demonstrate RUS project quality.
        </p>
      </section>
