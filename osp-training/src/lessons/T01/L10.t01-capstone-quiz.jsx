// Net-new — T01 Fundamentals & Vocabulary, Lesson 10 (Capstone Review)
// D031: graded inline Quiz retired — the T01-final pool is the graded capstone gate now.
// T01.L10 — T01 Capstone Review
// ⚠ URGENT priority (CEO dispatch): T01 is LIVE and this quiz was ungated + graded.

import React from 'react';
import LessonLayout from '../../components/LessonLayout.jsx';

export const meta = {
  id: 'T01.L10',
  course_id: 'T01',
  title: 'T01 Capstone Review — Fundamentals & Vocabulary',
  order: 10,
  lesson_type: 'capstone-review',
  completesOnView: true,
  prerequisites: [
    'T01.L01', 'T01.L02', 'T01.L03', 'T01.L04',
    'T01.L05', 'T01.L06', 'T01.L07', 'T01.L08', 'T01.L09',
  ],
  vocabulary_introduced: [],
  learning_objectives: [
    'Identify and distinguish all 30+ vocabulary terms introduced across T01 lessons',
    'Match each acronym to its full expansion and explain its role in OSP field work',
    'Apply knowledge of pole anatomy, cable components, and splice-case internals to scenario questions',
    'Demonstrate mastery of foundational OSP concepts through a cumulative 15-question assessment',
  ],
  vocabulary_assumed: [
    { term: 'OSP', source_lesson_id: 'T01.L01' },
    { term: 'ISP', source_lesson_id: 'T01.L01' },
    { term: 'demarcation point', source_lesson_id: 'T01.L01' },
    { term: 'OLT', source_lesson_id: 'T01.L01' },
    { term: 'ONT', source_lesson_id: 'T01.L01' },
    { term: 'attachment', source_lesson_id: 'T01.L02' },
    { term: 'sag', source_lesson_id: 'T01.L02' },
    { term: 'span', source_lesson_id: 'T01.L02' },
    { term: 'climbing space', source_lesson_id: 'T01.L02' },
    { term: 'communication space', source_lesson_id: 'T01.L02' },
    { term: 'sheath', source_lesson_id: 'T01.L03' },
    { term: 'buffer tube', source_lesson_id: 'T01.L03' },
    { term: 'ripcord', source_lesson_id: 'T01.L03' },
    { term: 'armor', source_lesson_id: 'T01.L03' },
    { term: 'messenger', source_lesson_id: 'T01.L03' },
    { term: 'splice case', source_lesson_id: 'T01.L04' },
    { term: 'splice tray', source_lesson_id: 'T01.L04' },
    { term: 'fan-out', source_lesson_id: 'T01.L04' },
    { term: 'gel seal', source_lesson_id: 'T01.L04' },
    { term: 'survey', source_lesson_id: 'T01.L05' },
    { term: 'make-ready', source_lesson_id: 'T01.L05' },
    { term: 'as-built', source_lesson_id: 'T01.L05' },
    { term: 'designer', source_lesson_id: 'T01.L06' },
    { term: 'splicer', source_lesson_id: 'T01.L06' },
    { term: 'PE', source_lesson_id: 'T01.L06' },
    { term: 'FDH', source_lesson_id: 'T01.L07' },
    { term: 'NAP', source_lesson_id: 'T01.L07' },
    { term: 'PON', source_lesson_id: 'T01.L07' },
    { term: 'strand map', source_lesson_id: 'T01.L07' },
    { term: 'OTDR', source_lesson_id: 'T01.L08' },
    { term: 'OLTS', source_lesson_id: 'T01.L08' },
    { term: 'MGN', source_lesson_id: 'T01.L08' },
    { term: 'IBT', source_lesson_id: 'T01.L08' },
    { term: 'GES', source_lesson_id: 'T01.L08' },
    { term: 'NESC', source_lesson_id: 'T01.L02' },
    { term: 'NEC', source_lesson_id: 'T01.L08' },
    { term: 'LOTO', source_lesson_id: 'T01.L08' },
    { term: 'ADSS', source_lesson_id: 'T01.L08' },
    { term: 'ROW', source_lesson_id: 'T01.L08' },
    { term: 'HDD', source_lesson_id: 'T01.L08' },
    { term: 'ITU-T', source_lesson_id: 'T01.L09' },
    { term: 'ICEA', source_lesson_id: 'T01.L09' },
    { term: 'FCC', source_lesson_id: 'T01.L09' },
    { term: 'USACE', source_lesson_id: 'T01.L09' },
  ],
  estimated_minutes: 30,
};

export default function T01L10_CapstoneQuiz() {
  return (
    <LessonLayout meta={meta}>

      <section data-tier="foundations">
        <h2>T01 Capstone Review — Fundamentals &amp; Vocabulary</h2>
        <p>
          T01.L01–L09 introduced 30+ core OSP vocabulary terms and acronyms — pole anatomy,
          cable and splice-case components, the project lifecycle, and the standards landscape.
          Before moving on to T18 (Safety &amp; OSHA), revisit any of those lessons for a term
          you're unsure of.
        </p>
        <p className="mt-2">
          The graded T01 capstone assessment — drawn from the full T01 pool — is the gate for
          the topic; this page is reading/review material, not a graded quiz.
        </p>
      </section>


    </LessonLayout>
  );
}
