// Net-new — T06.L11 Underground Design QA Checklist
// Hands-on-walkthrough lesson: design review, depth marking on plan,
// fill calc verification, access spacing, QA branching scenario.
// Sources: net-new; synthesizes all prior T06 lessons.

import React from 'react';
import LessonLayout from '../../components/LessonLayout.jsx';
import BranchingScenario from '../../components/primitives/BranchingScenario.jsx';
import AnnotatedDiagram from '../../components/primitives/AnnotatedDiagram.jsx';
import Quiz from '../../components/primitives/Quiz.jsx';
import Flashcard from '../../components/Flashcard.jsx';

export const meta = {
  id: 'T06.L11',
  course_id: 'T06',
  title: 'Underground Design QA Checklist',
  order: 11,
  lesson_type: 'working',
  prerequisites: [
    'T06.L01', 'T06.L02', 'T06.L03', 'T06.L04',
    'T06.L05', 'T06.L06', 'T06.L07', 'T06.L08',
    'T06.L09', 'T06.L10',
  ],
  learning_objectives: [
    'Apply the T06 QA checklist to a sample underground design plan and identify non-compliances',
    'Recognize the four most common underground design errors: missing depth callout, fill overrun, pedestal spacing violation, separation deficiency',
    'Explain the difference between a design drawing check and a field inspection check',
    'Complete a walkthrough of a sample design cross-section and call out each error',
  ],
  estimated_minutes: 20,
  vocabulary_introduced: [
    'design review',
    'depth marking on plan',
    'fill calc verification',
    'access spacing',
  ],
};

export const vocabulary_introduced = {
  'design review': 'A structured examination of a design drawing or plan set before construction begins, performed to verify that every design element meets applicable codes (NESC, NEC, RUS), project specifications, and engineering standards. A proper design review is performed by someone OTHER than the person who drew the plan — independent eyes catch different classes of error. In OSP work, the design review typically checks: burial depth callouts, conduit sizing, fill percentages, access spacing, separation from foreign utilities, bore path profiles, and materials specification.',
  'depth marking on plan': 'The notation on a design drawing that specifies the exact burial depth at key points along the route — particularly at road crossings, utility crossings, changes in soil type, and driveway crossings. A plan that shows conduit route without depth callouts at these critical points is incomplete: the bore crew and inspector cannot verify compliance with RUS 1751F-635 cover requirements without an explicit depth reference on the drawing. Standard format: a circled depth callout (e.g., "36" cover") at each critical point with an arrow to the relevant plan element.',
  'fill calc verification': 'The process of checking that the conduit fill percentage (cable or innerduct cross-sectional area as a fraction of conduit internal area) does not exceed 40% at any point in the design. Fill calc verification requires knowing: the conduit schedule and inner diameter, the cable or innerduct outer diameter, and the count of cables or innerducts installed in each conduit. A plan that specifies conduit size and cable count without a fill calculation is incomplete — it requires the reviewer to independently compute fill to confirm compliance.',
  'access spacing': 'The measured distance between successive terminal pedestals, manholes, or handholes along the underground route, verified against the RUS 1751F-635 maximum spacing rule of 330 feet. Access spacing must be verified along the actual cable route (not point-to-point straight-line distance). A design drawing should include dimensioned route segments between each access point so the reviewer can confirm that no segment exceeds the 330-foot maximum.',
};

export const vocabulary_assumed = [
  { term: 'terminal pedestal', source_lesson_id: 'T06.L08' },
  { term: 'conduit fill', source_lesson_id: 'T06.L04' },
  { term: 'pedestal spacing', source_lesson_id: 'T06.L08' },
  { term: 'minimum cover', source_lesson_id: 'T06.L02' },
  { term: 'supply-communication separation', source_lesson_id: 'T06.L09' },
  { term: 'RUS 1751F-635', source_lesson_id: 'T06.L02' },
  { term: 'innerduct traceability', source_lesson_id: 'T06.L10' },
];

export const key_terms = Object.entries(vocabulary_introduced).map(([term, definition]) => ({
  term,
  definition,
}));

export default function T06L11_UndergroundDesignQAChecklist() {
  return (
    <LessonLayout meta={meta}>

      {/* ── FOUNDATIONS ─────────────────────────────────────────────────── */}
      <section data-tier="foundations">
        <h2>In Plain English</h2>
        <p>
          A design drawing is only as good as the QA (quality assurance) check performed
          on it before anyone starts digging. Mistakes on drawings — wrong depth, forgotten
          pedestal, conduit that's too small — are cheap to fix on paper. The same mistakes
          caught after the conduit is buried are expensive: open-cut the road, pull the
          conduit, start over.
        </p>
        <p className="mt-2">
          This lesson walks you through a structured checklist for reviewing an underground
          fiber design. Every item on the checklist corresponds to a requirement you've
          learned in T06.L01 through T06.L10. Think of it as the final exam you apply to
          every design before you sign off on it.
        </p>
        <p className="mt-2">
          The four most common underground design errors, in order of frequency:
        </p>
        <ol className="list-decimal pl-5 space-y-2 mt-2 text-slate-300/90">
          <li><strong>Missing depth callout</strong> — the drawing shows the conduit route but doesn't specify burial depth at road crossings or utility crossings</li>
          <li><strong>Fill overrun</strong> — the conduit schedule is too small for the cable count, or innerducts are sized incorrectly</li>
          <li><strong>Pedestal spacing violation</strong> — the route between two pedestals exceeds 330 feet along the cable path</li>
          <li><strong>Separation deficiency</strong> — the route runs too close to an electric supply cable, violating NESC §32 or §35 separation requirements</li>
        </ol>

        {/* Key Terms Flashcards */}
        <Flashcard
          deckId="T06-L11"
          cards={[
            {
              id: 'T06-L11-fc-review',
              front: 'What is a design review in OSP underground work?',
              back: 'A structured examination of a design drawing or plan set before construction begins, performed to verify that every design element meets applicable codes (NESC, NEC, RUS), project specifications, and engineering standards. A proper design review is performed by someone OTHER than the person who drew the plan. In OSP work, the design review typically checks: burial depth callouts, conduit sizing, fill percentages, access spacing, separation from foreign utilities, bore path profiles, and materials specification.',
            },
            {
              id: 'T06-L11-fc-depth',
              front: 'What is a depth marking on plan and why is it required?',
              back: 'The notation on a design drawing that specifies the exact burial depth at key points along the route — particularly at road crossings, utility crossings, changes in soil type, and driveway crossings. A plan that shows conduit route without depth callouts at these critical points is incomplete: the bore crew and inspector cannot verify compliance with RUS 1751F-635 cover requirements without an explicit depth reference on the drawing.',
            },
            {
              id: 'T06-L11-fc-fill',
              front: 'What is fill calc verification in a design review?',
              back: 'The process of checking that the conduit fill percentage (cable or innerduct cross-sectional area as a fraction of conduit internal area) does not exceed 40% at any point in the design. Fill calc verification requires knowing: the conduit schedule and inner diameter, the cable or innerduct outer diameter, and the count of cables or innerducts installed in each conduit. A plan that specifies conduit size and cable count without a fill calculation is incomplete.',
            },
            {
              id: 'T06-L11-fc-spacing',
              front: 'What is access spacing verification in underground design QA?',
              back: 'The measured distance between successive terminal pedestals, manholes, or handholes along the underground route, verified against the RUS 1751F-635 maximum spacing rule of 330 feet. Access spacing must be verified along the actual cable route (not point-to-point straight-line distance). A design drawing should include dimensioned route segments between each access point so the reviewer can confirm that no segment exceeds the 330-foot maximum.',
            },
          ]}
        />
      </section>

      {/* ── WORKING ─────────────────────────────────────────────────────── */}
      <section data-tier="working">
        <h2>The T06 QA Checklist</h2>

        <p>
          Use this checklist on every underground fiber design before issuing for construction.
          Each item maps to a specific T06 lesson and standard. Check them in order — the
          first four items catch the most common errors.
        </p>

        <div className="mt-4 space-y-4">
          {/* Item 1 */}
          <div className="p-4 border border-white/10 rounded-lg bg-white/3">
            <p className="font-semibold text-slate-100">1. Burial depth — callouts at ALL critical points</p>
            <p className="text-sm text-slate-300/90 mt-1">
              Required callouts: every road crossing, every driveway crossing, every utility crossing,
              every change in soil type or AHJ zone, every bore path segment where depth changes.
              Verify each callout meets the applicable depth rule (RUS 1751F-635 §6 minimums; AHJ
              may require more). Flag any road crossing without a depth callout as INCOMPLETE.
              (Source: RUS 1751F-635 §6; T06.L02.)
            </p>
          </div>

          {/* Item 2 */}
          <div className="p-4 border border-white/10 rounded-lg bg-white/3">
            <p className="font-semibold text-slate-100">2. Conduit fill percentage — all conduit segments</p>
            <p className="text-sm text-slate-300/90 mt-1">
              For each conduit segment on the drawing: confirm conduit schedule (ID known),
              cable or innerduct count and OD known, fill % computed, fill ≤ 40%. If fill is
              not shown on the drawing, compute it independently. Flag any segment over 40%
              as NON-COMPLIANT. (Source: T06.L04; 40% fill rule.)
            </p>
          </div>

          {/* Item 3 */}
          <div className="p-4 border border-white/10 rounded-lg bg-white/3">
            <p className="font-semibold text-slate-100">3. Pedestal spacing — along the cable route</p>
            <p className="text-sm text-slate-300/90 mt-1">
              Measure the cable-route distance (not straight-line) between each pair of successive
              pedestals. Maximum = 330 feet (RUS 1751F-635 §7). Also verify pedestals are placed
              at ALL branch points and service drop locations. Flag any segment over 330 feet as
              NON-COMPLIANT. (Source: RUS 1751F-635 §7; T06.L08.)
            </p>
          </div>

          {/* Item 4 */}
          <div className="p-4 border border-white/10 rounded-lg bg-white/3">
            <p className="font-semibold text-slate-100">4. Separation from foreign utilities — crossings and parallels</p>
            <p className="text-sm text-slate-300/90 mt-1">
              For each utility crossing shown on the design: confirm the vertical separation meets
              NESC §35 minimum (cable in conduit) or §32 (direct-buried), and that field practice
              12-inch minimum is honored at crossings with electric supply. For parallel runs:
              confirm 6-inch minimum separation in the same corridor. Flag any crossing without
              separation callout as INCOMPLETE. (Source: NESC C2 §32/§35 [confirm edition];
              T06.L09.)
            </p>
          </div>

          {/* Item 5 */}
          <div className="p-4 border border-white/10 rounded-lg bg-white/3">
            <p className="font-semibold text-slate-100">5. Conduit material specification — AML listing confirmed</p>
            <p className="text-sm text-slate-300/90 mt-1">
              Verify the materials list specifies RUS AML-listed conduit and innerduct where
              required. For RUS-funded projects: innerduct must reference RUS 1751F-643 AML
              listing. Flag any material specification without an AML reference as PENDING
              VERIFICATION. (Source: RUS 1751F-643; T06.L10.)
            </p>
          </div>

          {/* Item 6 */}
          <div className="p-4 border border-white/10 rounded-lg bg-white/3">
            <p className="font-semibold text-slate-100">6. HDD bore profiles — depth, radius, exit pit location</p>
            <p className="text-sm text-slate-300/90 mt-1">
              For every bore shot: confirm the bore profile shows entry angle, target depth,
              exit angle, and that the profile respects minimum cover at all crossings. Confirm
              exit pit location does not conflict with utilities or paved surface where open-cut
              would require a separate permit. (Source: T06.L07; RUS 1751F-635 §6.)
            </p>
          </div>

          {/* Item 7 */}
          <div className="p-4 border border-white/10 rounded-lg bg-white/3">
            <p className="font-semibold text-slate-100">7. Warning tape — specified on all buried segments</p>
            <p className="text-sm text-slate-300/90 mt-1">
              Confirm the design calls out warning tape (orange / telecom color per APWA) at
              6–12 inches above conduit for all buried segments. Flag segments without warning
              tape callout as INCOMPLETE. (Source: APWA Utility Color Code; T06.L09; CGA
              Best Practices v19.)
            </p>
          </div>

          {/* Item 8 */}
          <div className="p-4 border border-white/10 rounded-lg bg-white/3">
            <p className="font-semibold text-slate-100">8. Slack loop specification — all pedestals and handholes</p>
            <p className="text-sm text-slate-300/90 mt-1">
              Confirm that the design or installation notes call out minimum slack storage
              (20–30 feet per cable end) at all terminal pedestals and splice handholes.
              (Source: RUS 1751F-635 §7; T06.L08.)
            </p>
          </div>
        </div>

        <h3 className="mt-6 font-semibold">Design drawing check vs. field inspection check</h3>
        <p>
          The QA checklist above is a <em>design drawing check</em> — done before construction
          starts, on paper (or the CAD file). A <em>field inspection check</em> is different:
          it's done during or after construction to verify that what was built matches the design.
        </p>
        <p className="mt-2">
          The two checks are related but catch different errors:
        </p>
        <ul className="list-disc pl-5 space-y-2 mt-2 text-sm text-slate-300/90">
          <li>Design check catches: wrong conduit size, missing pedestal, depth too shallow on paper</li>
          <li>Field check catches: conduit buried at wrong depth, bore path wandered from profile, pedestal placed in wrong location, unlisted innerduct substituted by contractor</li>
        </ul>
        <p className="mt-2 text-sm">
          Both checks are required on RUS-funded projects. The engineer of record typically
          performs or reviews the design check before issuing for bid. The inspector performs
          the field check during construction inspection visits and during the project closeout
          inspection. (Source: RUS 1751F-635 §11; field practice.)
        </p>
      </section>

      {/* ── ADVANCED ────────────────────────────────────────────────────── */}
      <section data-tier="advanced">
        <h2>Advanced: River and Wetland Crossing Special Checks</h2>

        <h3 className="mt-4 font-semibold">Extra QA items for waterway crossings</h3>
        <p>
          When the underground route includes a bore under a river, creek, or wetland,
          additional QA items apply beyond the standard checklist:
        </p>
        <ol className="list-decimal pl-5 space-y-2 mt-2 text-sm">
          <li>
            <strong>USACE NWP authorization confirmed.</strong> Waterway crossings typically
            require either NWP 57 (telecommunications) authorization or a Section 404 individual
            permit. Verify the applicable permit is issued and conditions are documented before
            construction starts. (Source: USACE 33 CFR Part 320-332; T04.L08.)
          </li>
          <li>
            <strong>Minimum cover under the waterway bed.</strong> Most NWP 57 authorizations
            require a minimum of 36–48 inches of cover below the deepest point of the waterway
            bed (scour depth). Verify the bore profile shows the required cover at the waterway
            crossing. [confirm with USACE district conditions on your permit]
          </li>
          <li>
            <strong>Frac-out contingency plan.</strong> The construction plan must include a
            frac-out response procedure for waterway crossings: what to do if slurry breaks
            through to the waterway, who to call, how to contain the spill. (Source: CGA Best
            Practices v19; NWP conditions; field practice.)
          </li>
          <li>
            <strong>Conduit sealed at both ends during bore.</strong> Verify the bore plan
            calls for conduit ends to be sealed before and after installation to prevent water
            from entering the conduit during the crossing construction. Water inside a bore-installed
            conduit under a river can migrate along the cable for miles after installation.
          </li>
        </ol>
      </section>

      {/* ── ANNOTATED DIAGRAM ────────────────────────────────────────────── */}
      <AnnotatedDiagram
        title="Sample Underground Design Plan — QA Walkthrough"
        description="Click each flagged point on this design drawing excerpt to see the QA error it illustrates. This is a composite example showing multiple common design deficiencies in one plan view."
        src="/training/diagrams/underground-design-qa-plan.svg"
        alt="Underground fiber design plan view showing four flagged QA error locations: missing depth callout at road crossing, fill overrun segment, pedestal spacing gap, and separation deficiency"
        aspectRatio={1.6}
        hotPoints={[
          {
            id: 'depth-miss',
            x: 30,
            y: 25,
            label: 'Error 1: Missing depth callout',
            type: 'click',
            explanation:
              'This road crossing has no burial depth specified. The bore crew will not know the required depth, and the inspector cannot verify compliance with RUS 1751F-635 §6. Fix: add "36\\" cover" annotation at this crossing with an arrow to the conduit line. This is the most common single error in underground designs.',
          },
          {
            id: 'fill-error',
            x: 55,
            y: 45,
            label: 'Error 2: Fill overrun',
            type: 'click',
            explanation:
              'This 2-inch conduit segment specifies three 1-inch innerducts. Fill check: ID of 2-inch Sch 40 PVC = 2.067 in. Area of THREE 1-inch innerducts = 3 × π × 0.5² = 2.36 in². Conduit area = π × 1.034² = 3.36 in². Fill = 2.36 / 3.36 = 70.2% — far over the 40% limit. Fix: upgrade to 3-inch conduit (ID = 3.230 in; fill = 2.36 / 8.19 = 28.8%) or reduce to two 1-inch innerducts.',
          },
          {
            id: 'spacing-error',
            x: 70,
            y: 65,
            label: 'Error 3: Pedestal spacing over limit',
            type: 'click',
            explanation:
              'The cable route distance between these two pedestals is 380 feet (measured along the cable path, accounting for the bend at the property corner). RUS 1751F-635 §7 maximum is 330 feet. Fix: add an intermediate pedestal at approximately 190 feet from each end of the segment. This brings both sub-segments to 190 feet — well under the 330-foot maximum.',
          },
          {
            id: 'separation-error',
            x: 20,
            y: 75,
            label: 'Error 4: Separation deficiency',
            type: 'click',
            explanation:
              'This crossing shows a communication conduit crossing an existing direct-buried electric supply at only 4 inches of vertical separation. NESC §35 may technically permit less than 12 inches at a conduit crossing, but field best practice is 12 inches minimum to account for ±18-inch locate tolerance. Fix: adjust the bore depth profile at this crossing to achieve 12 inches of vertical separation.',
          },
        ]}
      />

      {/* ── BRANCHING SCENARIO ────────────────────────────────────────────── */}
      <BranchingScenario
        id="T06-L11-BS1"
        title="QA Walkthrough: Find the Design Error"
        description="You're reviewing a 1,200-foot underground fiber route design for a residential subdivision in Georgia before issuing it for bid. The route uses 3-inch Schedule 40 PVC with two 1.25-inch HDPE innerducts. You have the plan in front of you. Walk through the QA checklist."
        nodes={[
          {
            id: 'start',
            type: 'decision',
            text: "Start with the depth callouts. The design shows two road crossings. One has a '36\" cover' annotation. The other road crossing has no depth annotation. What do you do?",
            choices: [
              { label: 'Issue for bid — one callout is probably enough and the crew will figure out the other', nextId: 'depth-bad' },
              { label: 'Return the drawing for revision — mark the missing road crossing depth callout as INCOMPLETE before issuing', nextId: 'depth-good' },
            ],
          },
          {
            id: 'depth-bad',
            type: 'outcome',
            text: "Wrong. Issuing a drawing with a missing depth callout at a road crossing means the bore crew has no documented required depth at that point. If they bore too shallow, you have a compliance problem. If the inspector notices and stops construction, you've paid for a crew idle day. Catch it on paper; fix takes 5 minutes.",
            variant: 'bad',
          },
          {
            id: 'depth-good',
            type: 'decision',
            text: "Correct. You marked the missing callout. Now check fill. The plan shows: 3-inch Schedule 40 PVC (ID = 3.230 inches), two 1.25-inch HDPE innerducts. Fill = 2 × π × 0.625² / (π × 1.615²) = 2 × 1.227 / 8.19 = 2.454 / 8.19 = 29.96%. Is this acceptable?",
            choices: [
              { label: 'No — 30% exceeds some fill standard I recall', nextId: 'fill-bad' },
              { label: 'Yes — 30% is well under the 40% limit; fill check passes', nextId: 'fill-good' },
            ],
          },
          {
            id: 'fill-bad',
            type: 'outcome',
            text: "Incorrect. 30% is well under the 40% maximum fill rule. The 40% limit exists to allow adequate slurry circulation around innerducts during installation and cable pull-back. 30% fill means 70% of the conduit cross-section is open — plenty of margin. This fill calc passes.",
            variant: 'bad',
          },
          {
            id: 'fill-good',
            type: 'decision',
            text: "Correct — fill passes at 30%. Now check pedestal spacing. The plan shows pedestals at: 0 ft (entry), 240 ft, 520 ft, 780 ft, 1,200 ft (end of route). The segment from 520 ft to 780 ft measures 260 feet along the cable route. From 780 ft to 1,200 ft measures 420 feet. Is pedestal spacing acceptable?",
            choices: [
              { label: 'Yes — all segments look fine to me', nextId: 'spacing-bad' },
              { label: 'No — the 420-foot segment from 780 ft to 1,200 ft exceeds the 330-foot maximum; needs an intermediate pedestal', nextId: 'spacing-good' },
            ],
          },
          {
            id: 'spacing-bad',
            type: 'outcome',
            text: "Missed it. The 420-foot segment from 780 ft to 1,200 ft is 90 feet over the 330-foot maximum. You need to add an intermediate pedestal — ideally at approximately the 990-foot mark to split the segment into 210 ft + 210 ft (both under 330 ft). If you catch this after construction, adding a mid-segment pedestal means open-cut, cable splice, and new pedestal installation — expensive.",
            variant: 'bad',
          },
          {
            id: 'spacing-good',
            type: 'outcome',
            text: "Correct! You caught the 420-foot spacing violation before issuing for bid. Adding a pedestal at the 990-foot mark (splitting the segment into 210 ft + 210 ft) resolves the issue. The revised design now passes all four major QA checks: depth callouts, fill, pedestal spacing, and separation. Issue for bid after the revisions are incorporated.",
            variant: 'good',
          },
        ]}
      />

      {/* ── PER-LESSON QUIZ ──────────────────────────────────────────────── */}
      <Quiz
        title="T06.L11 Check — Underground Design QA"
        mode="multiple-choice"
        questions={[
          {
            id: 'T06-L11-Q1',
            type: 'mc',
            prompt: 'Which of the following is the most common single error found in underground fiber design plans?',
            choices: [
              'Conduit fill percentage over 40%',
              'Missing burial depth callout at road crossings',
              'Innerduct material not AML-listed',
              'Pedestal spacing over 330 feet',
            ],
            answerIndex: 1,
            explanation: 'Missing depth callouts at road crossings is the most common single error. A plan can show the conduit route without ever specifying the required burial depth — which means the bore crew and inspector have no documented depth to verify against. Depth callouts at road crossings, utility crossings, and grade transitions are required before a plan is issued for construction. (Source: field practice; RUS 1751F-635 §6; T06.L02.)',
          },
          {
            id: 'T06-L11-Q2',
            type: 'mc',
            prompt: 'A design drawing shows a route segment from pedestal at stationing 620+00 to pedestal at stationing 623+50. The route follows the road shoulder with a 30-foot offset around a drainage culvert. What is the measured segment length, and does it comply with RUS 1751F-635 §7?',
            choices: [
              '350 feet along route (stationing difference); non-compliant — exceeds 330-foot maximum',
              '350 feet but pedestal spacing is measured point-to-point, so this passes at 320 feet straight-line',
              '350 feet along route; compliant — the 330-foot rule has a 10% tolerance',
              '350 feet along route; the culvert offset adds 30 feet → 380 feet total; non-compliant',
            ],
            answerIndex: 3,
            explanation: 'Pedestal spacing is measured along the cable route, not straight-line. Stationing 620+00 to 623+50 = 350 feet of stationing (straight-line along the road). But the route offsets 30 feet around the culvert, adding approximately 30 feet of additional route length (the two sides of the offset). Total cable route distance ≈ 380 feet — 50 feet over the 330-foot maximum. Add an intermediate pedestal. There is no 10% tolerance; 330 feet is the hard limit. (Source: RUS 1751F-635 §7; T06.L08.)',
          },
          {
            id: 'T06-L11-Q3',
            type: 'mc',
            prompt: 'A design drawing specifies 4 × 1.25-inch HDPE innerducts in a 4-inch Schedule 40 PVC outer conduit (ID = 4.026 inches). Is this design compliant with the 40% fill rule?',
            choices: [
              'Yes — 4-inch conduit easily accommodates four 1.25-inch innerducts',
              'No — fill = 4 × π × 0.625² / (π × 2.013²) = 4 × 1.227 / 12.73 = 38.5% — barely under 40%, acceptable',
              'No — the fill exceeds 40% for this configuration',
              'Yes — the 40% rule applies only to fiber cable inside innerduct, not innerduct inside conduit',
            ],
            answerIndex: 1,
            explanation: 'Fill = 4 × (π × 0.625²) / (π × 2.013²) = 4 × 1.2272 / 12.730 = 4.909 / 12.730 = 38.6%. This is under the 40% maximum — the design is compliant, but just barely. A reviewer would note this as "tight" and flag it as a potential issue if cable OD is larger than spec. Sanity check: 38.6% fill leaves 61.4% of the conduit cross-section open — adequate for slurry circulation during bore. (Source: T06.L04; fill rule.)',
          },
          {
            id: 'T06-L11-Q4',
            type: 'mc',
            prompt: 'What is the key difference between a design drawing check and a field inspection check?',
            choices: [
              'Design drawing check is done by the RUS district office; field inspection is done by the engineer of record',
              'Design drawing check verifies the plan before construction; field inspection verifies that what was built matches the design',
              'Design drawing check only verifies conduit fill; field inspection checks all other items',
              'They are the same process performed at different project phases',
            ],
            answerIndex: 1,
            explanation: 'A design drawing check is a pre-construction review of the plan — catching errors on paper before anyone digs. A field inspection is a during/post-construction check verifying that actual installation matches the design: conduit depth as-built, bore path actual vs. planned, materials installed match specification, pedestal locations per design. Both are required on RUS-funded projects. They catch different classes of errors because you can\'t see the conduit on paper when it\'s underground. (Source: RUS 1751F-635 §11; field practice.)',
          },
        ]}
      />

    </LessonLayout>
  );
}
