...
export const meta = {
  ...
  vocabulary_assumed: [
    ...
    { term: 'design review', source_lesson_id: 'T06.L11' },
...
export default function T06L12_CapstoneQuiz() {
  return (
    <LessonLayout meta={meta}>
      ...
      <WorkedExample
        title="Capstone Design Check — Conduit Fill and Pull Tension"
        description="A 1,200-foot HDD bore under a residential road will install three 1.25-inch HDPE innerducts inside a 4-inch Schedule 40 PVC outer conduit (ID = 4.026 inches). The route has two 90° bends and one 45° bend. Cable mass = 0.10 lb/ft. Coefficient of friction µ = 0.35. Confirm fill compliance and estimate maximum pull tension."
        variables={[
          { key: 'condID', label: 'Conduit ID', units: 'inches', default: 4.026, min: 2, max: 6, step: 0.001 },
          { key: 'idOD', label: 'Innerduct OD', units: 'inches', default: 1.25, min: 0.75, max: 2, step: 0.05 },
          { key: 'idCount', label: 'Innerduct count', units: '', default: 3, min: 1, max: 6, step: 1 },
          { key: 'length', label: 'Pull length', units: 'ft', default: 1200, min: 100, max: 5000, step: 50 },
          { key: 'mass', label: 'Cable mass per foot', units: 'lb/ft', default: 0.10, min: 0.02, max: 0.5, step: 0.01 },
          { key: 'mu', label: 'Coefficient of friction (µ)', units: '', default: 0.35, min: 0.20, max: 0.60, step: 0.01 },
          { key: 'bends90', label: 'Number of 90° bends', units: '', default: 2, min: 0, max: 10, step: 1 },
          { key: 'bends45', label: 'Number of 45° bends', units: '', default: 1, min: 0, max: 10, step: 1 },
        ]}
        formula={({ condID, idOD, idCount, length, mass, mu, bends90, bends45 }) => {
          // Fill calculation
          const condArea = Math.PI * Math.pow(condID / 2, 2);
          const idArea = Math.PI * Math.pow(idOD / 2, 2) * idCount;
          const fillPct = (idArea / condArea) * 100;

          // Pull tension: T = W * L * mu * e^(mu * total_angle_radians)
          // Total bend angle in radians
          const totalRadians = bends90 * (Math.PI / 2) + bends45 * (Math.PI / 4);
          const W = mass * length; // total cable weight
          const T = W * mu * Math.exp(mu * totalRadians);

          return { fillPct, T, fillPct_raw: fillPct };
        }}
        steps={({ condID, idOD, idCount, length, mass, mu, bends90, bends45 }, result) => {
          const condArea = Math.PI * Math.pow(condID / 2, 2);
          const idArea = Math.PI * Math.pow(idOD / 2, 2) * idCount;
          const fillPct = (idArea / condArea) * 100;
          const totalDeg = bends90 * 90 + bends45 * 45;
          const totalRad = totalDeg * Math.PI / 180;
          const W = mass * length;
          const T = W * mu * Math.exp(mu * totalRad);

          return [
            { expression: `Step 1 — Conduit cross-section area: π × (${condID}/2)² = π × ${(condID/2).toFixed(3)}² = ${condArea.toFixed(3)} in²` },
            { expression: `Step 2 — Single innerduct area: π × (${idOD}/2)² = π × ${(idOD/2).toFixed(3)}² = ${(Math.PI * Math.pow(idOD/2,2)).toFixed(3)} in²` },
            { expression: `Step 3 — Total innerduct area: ${idCount} × ${(Math.PI * Math.pow(idOD/2,2)).toFixed(3)} = ${idArea.toFixed(3)} in²` },
            { expression: `Step 4 — Fill% = (${idArea.toFixed(3)} / ${condArea.toFixed(3)}) × 100`, value: fillPct, unit: '%' },
            { expression: fillPct <= 40 ? `Fill check: ${fillPct.toFixed(1)}% ≤ 40% → PASS` : `Fill check: ${fillPct.toFixed(1)}% > 40% → FAIL — reduce innerduct count or upsize conduit` },
            { expression: `Step 5 — Pull tension: Total cable weight W = ${mass} lb/ft × ${length} ft = ${W.toFixed(1)} lb` },
            { expression: `Step 6 — Total bend angle = ${bends90} × 90° + ${bends45} × 45° = ${totalDeg}° = ${totalRad.toFixed(4)} radians` },
            { expression: `Step 7 — T = W × µ × e^(µ × θ) = ${W.toFixed(1)} × ${mu} × e^(${mu} × ${totalRad.toFixed(4)})` },
            { expression: `Step 8 — e^(${(mu * totalRad).toFixed(4)}) = ${Math.exp(mu * totalRad).toFixed(4)}` },
            { expression: `Step 9 — T = ${W.toFixed(1)} × ${mu} × ${Math.exp(mu * totalRad).toFixed(4)}`, value: T, unit: 'lb' },
          ];
        }}
        sanityCheck={(result) => {
          const fillPct = result.fillPct;
          const T = result.T;
          return `Fill = ${fillPct.toFixed(1)}% (${fillPct <= 40 ? 'PASS — under 40% limit' : 'FAIL — exceeds 40% limit'}). Pull tension estimate = ${T.toFixed(0)} lb. Compare to your specific cable manufacturer's rated maximum pull tension — exceed it and the cable may be damaged inside the conduit.`;
        }}
        resultLabel="Pull tension estimate"
        resultUnit="lb"
        resultDecimals={0}
      />

      {/* CAPSTONE BRANCHING SCENARIO */}
      <BranchingScenario
        id="T06-L12-BS1"
        title="Capstone Scenario: Design a 1-Mile Underground Route"
        description="You're designing a 1-mile underground fiber route in a Georgia residential subdivision. Route profile: 40% residential lawns (ROW available), 20% wetland crossing (requires bore), 40% paved residential road. Soil: clay. Existing utilities: electric supply direct-buried parallel 20 feet north of centerline. Budget is standard RUS-funded residential. Work through the key design decisions."
        nodes={[
          {
            id: 'start',
            type: 'decision',
            text: "First decision: construction method for the 40% residential lawn segments (non-paved, ROW available, clay soil, no wetlands). What method do you specify?",
            choices: [
              { label: 'HDD — safest choice for any underground work', nextId: 'method-hdd' },
              { label: 'Open-cut trench — appropriate for residential ROW with available access', nextId: 'method-opencut' },
              { label: 'Plowing — fastest and cheapest for residential clay soil ROW', nextId: 'method-plow' },
            ],
          },
          {
            id: 'method-hdd',
            type: 'outcome',
            text: "Technically possible but not optimal. HDD costs 3–5× open-cut for open-field residential ROW without obstacles. Reserve HDD for the wetland crossing and any road crossings where open-cut would require costly road repair. For open residential ROW in clay soil with available access, open-cut or plow is the right choice.",
            variant: 'bad',
          },
          {
            id: 'method-opencut',
          ...
          },
          {
            id: 'wetland-shallow',
            type: 'outcome',
            text: "Wrong. USACE NWP 57 (telecommunications activities) typically requires 36–48 inches of cover below the waterway bed's scour depth — not 18 inches. 18 inches of cover is for residential non-traffic soil, not waterway crossings. Under-depth on a waterway crossing is a permit violation and creates a risk of cable damage from dredging or waterway scouring. [confirm with your USACE district permit conditions]",
            variant: 'bad',
          },
          {
            id: 'wetland-deep',
