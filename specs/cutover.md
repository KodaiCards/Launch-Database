...
1. **2.2 Legacy cleanup (first, clears the field):** kill AI assistant, audit-log feature (+ its 11 red tests — the L-015 baseline shrinks), setting-requests, legacy inspection/permits routes. Unwire from server.js → grep-verify nothing live references them → delete (~250KB). Also delete the mount-shadowed dead `GET /api/billing/report` in billing.js (O19).
