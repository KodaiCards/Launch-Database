...
**UV-cure rollable ribbon (intermittent bond).** A newer ribbon construction where the fibers are bonded at intervals rather than along the full ribbon length. Between bond points, the ribbon can be "rolled" into a round cross-section for tighter tube-fill ratios. At splice points, the ribbon must be "unrolled" to its flat form before stripping. Matrix removal follows the same procedure as standard UV-cured ribbon after unrolling.
