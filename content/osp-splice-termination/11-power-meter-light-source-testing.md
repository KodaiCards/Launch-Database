...
*Rationale:*
- **A — Incorrect.** The three reference methods produce different measured insertion loss values because they calibrate out different numbers of connector connections. The choice of reference method is not arbitrary — each method includes or excludes different connectors from the measurement, resulting in up to approximately 0.6 dB difference between methods on a typical link. [ANSI/TIA-526-7 §5; BICSI OSP-DRD Manual, Ch. 9]
- **B — Incorrect.** 0.3 dB (one connector pair) would be the difference only if Method C excluded a single end connector relative to Method B. In fact, Method C excludes both FUT end connectors — the connector at the launch end and the connector at the receive end — because both are zeroed into the three-jumper reference. Method B includes both end connectors. The delta is two connector pairs, not one. See the table in this lesson: Method B reports ~1.8 dB and Method C reports ~1.2 dB on the same 1.2 dB link — a difference of 0.6 dB. [ANSI/TIA-526-7 §5; BICSI OSP-DRD Manual, Ch. 9]
- **C — Correct.** Method C (three-jumper reference) zeroes out both end connectors of the fiber under test — the far-end launch-to-FUT connector and the FUT-to-receive connector — by including them in the reference path. Method B (two-jumper reference) zeros out the reference jumper connections only, leaving both FUT end connectors in the measured loss. The difference is two connector pairs × 0.3 dB = **0.6 dB**. This is confirmed by the lesson reference table (Method B ~1.8 dB, Method C ~1.2 dB). Using Method C when Method B is required understates link loss by ~0.6 dB — links near the budget limit may falsely appear to pass. [ANSI/TIA-526-7 §5; BICSI OSP-DRD Manual, Ch. 9]
- **D — Incorrect.** Method C produces lower measured insertion loss, not higher. By calibrating out both FUT end connectors in the reference step, Method C understates the total link loss relative to Method B. [ANSI/TIA-526-7 §5]
