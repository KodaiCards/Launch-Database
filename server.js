...
// The still-inline AI tools block (csv_smart_import case in
// executeTool, further below) calls a handful of CSV parsing helpers
// + the csvStage Map at module scope. Re-expose all of them so that
// block keeps working until routes/ai.js extraction lands. Drop these
// declarations when the AI extraction completes.
const _csvHelpers = require('./routes/hours_csv')._helpers;
const { normalizeName, normalizeWO, detectColumns, parseDateCell } = _csvHelpers;
const { csvStage, CSV_STAGE_TTL_MS } = require('./routes/_csv_stage');
// Also pin the parsing helpers on app.locals so route modules that
// need them can grab them without a hard require on hours_csv.
app.locals.csvHelpers = _csvHelpers;
