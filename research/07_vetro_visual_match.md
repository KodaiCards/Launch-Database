...
## Section 10 — Splice Canvas + PDF: Additional Implementation Notes

### Splice canvas keyboard shortcuts

To match VETRO's productivity-focused UX ("reduces complexity and shortens project timelines"), implement these keyboard shortcuts in the Konva splice editor:

- `Tab` — advance focus to next fiber slot in current tray
- `Shift+Tab` — previous fiber slot
- `Enter` — initiate splice connection from selected fiber
- `Escape` — cancel in-progress splice / deselect
- `Delete` / `Backspace` — remove selected splice
- `Ctrl+Z` / `Cmd+Z` — undo last splice action
- `+` / `-` — zoom canvas in/out

### PDF splice report — recommended field columns

For the per-closure table in the splice report PDF, use these columns in order:

| Column | Width | Notes |
|---|---|---|
| Tray | 6% | Tray number |
| Fiber # | 6% | Position within tray |
| In Color | 8% | TIA color swatch + name |
| In Cable | 18% | Cable name/ID |
| In Tube | 8% | Buffer tube color |
| Splice Type | 10% | Fusion / Mechanical |
| Out Cable | 18% | Cable name/ID |
| Out Tube | 8% | Buffer tube color |
| Out Color | 8% | TIA color swatch + name |
| Circuit | 10% | Circuit ID if assigned |

Print in portrait orientation. Use 8pt font for table rows. Header row background: `--vetro-primary` (#003F72) with white text. Alternating row background: white / #F4F5F7. `(inferred — TBD-VERIFY)`

### Splice report cover page template

```
[VETRO FiberMap logo top-left]        [Report date top-right]

FIBER SPLICE REPORT
─────────────────────────────────────
Network:       Anytown Fiber ISP
Service Loc:   123 Main Street
Report ID:     SPL-2024-001234
Generated By:  J. Smith
Generated:     2024-11-15 14:32 UTC

NETWORK PATH SUMMARY
─────────────────────────────────────
Hub/CO:        Central Office #1
Splice Points: 3 closures in path
Total Fibers:  96
Assigned:      48
Available:     48

[Map thumbnail showing route from hub to service location]
```

`(inferred from standard splice report conventions and VETRO "splice report" workflow description — TBD-VERIFY)`
