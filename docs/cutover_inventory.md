...
- **(b2) Billing → keystone** — cluster billing still bills `projects`; must move to the SA-bill path. **Design drafted → `docs/billing_keystone_design.md`** (awaiting Carter's 4 decisions). Needs migration 0066 (invoice↔service_area/EC link) + batch-bill/unbilled/report endpoints; also fixes the task-34 program-revenue attribution. PDF/send stays HOLD (#3).
