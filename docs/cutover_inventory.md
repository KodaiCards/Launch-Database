...
- **(b2) Billing → keystone** — **BACKEND DONE 2026-06-24** (migration 0066 + `routes/billing_keystone.js`: progressive ledger worklist/run/report/period-close; integration-tested). Design + status in `docs/billing_keystone_design.md`. **Remaining: billing UI** → C2 R12; plus a small follow-up to repoint program-financials at the new invoice↔SA link. PDF/send stays HOLD (#3).
- **(c) Projections** [#5] — **BACKEND DONE 2026-06-24** (`docs/projections_design.md`; migrations 0067/0068 + `routes/projections.js`: expected−billed per job, RUS budget burn, modular rollup, map data endpoint; completed→final lifecycle; tested). UI = C2 R13 (Money tab + Service Areas map tab + workspace lifecycle actions). Remaining #5 bits: permits/inspection tracking rework (separate from projections).
- **(d) Budgets redesign+port** [#4] — **BACKEND BUILT 2026-06-25** (`docs/budgets_design.md`; migration 0073 `service_area_jobs.budget_code_id` + reworked `/api/budgets/:id/summary` utilization on the keystone: billed + projected per RUS code; tested). Remaining: utilization VIEW in Money + EC-panel budget setup (C2 UI) + optional code auto-suggest.
