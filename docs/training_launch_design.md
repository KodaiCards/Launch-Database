...
- **Salvage outcome:** C2's one RT report was the only artifact worth keeping. It credibly flags **real accuracy problems in LIVE (pre-R18) content** (confirmed present on main: "Format V", AIA §3.3.1, NESC §01C/Section 26/Map 1, FCC Part 32 hierarchy, IEEE 81 section swap, paywalled quiz values). Preserved on main as `osp-training/docs/red-team-reports/T13-T22-SUSPECTED-live-errors-UNVERIFIED.md` — **labeled UNVERIFIED; the proposed fixes are NOT trusted.** No content edits were hand-applied. It's a **Priority-0 punch-list for the fresh instance (`briefs/claude-5.md`)** to independently verify + fix through the gate. The fresh, gated training-curriculum instance brief is `briefs/claude-5.md` (branch `claude-5/training-curriculum` off clean main).
